...
export function renderSkills(props: SkillsProps) {
  ...
  return html`
    <section class="card">
      ...
      ${filtered.length === 0
        ? html`<div class="muted" style="margin-top: 16px;">未找到相关技能。</div>`
        : html`
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const missing = [
    ...
    ...skill.missing.env.map((e) => `变量:${e}`),
    ...skill.missing.config.map((c) => `配置:${c}`),
    ...skill.missing.os.map((o) => `系统:${o}`),
  ];
  const reasons: string[] = [];
  if (skill.disabled) reasons.push("已停用");
      ...
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${skill.eligible ? "chip-ok" : "chip-warn"}">
            ${skill.eligible ? "符合运行条件" : "运行受限制"}
          </span>
          ${skill.disabled ? html`<span class="chip chip-warn">已停用</span>` : nothing}
        </div>
        ${missing.length > 0
          ? html`
