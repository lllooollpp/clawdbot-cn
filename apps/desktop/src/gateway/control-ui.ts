...
function resolveControlUiRoot(): string | null {
  ...
  const candidates = [
    // Packaged app root (inside app.asar or next to exe)
    path.resolve(here, "../../control-ui"),
    // Resources folder
    path.resolve(here, "../../resources/control-ui"),
    // Alongside executable
    execDir ? path.resolve(execDir, "control-ui") : null,
    // Electron resourcesPath (common in packaged apps)
    typeof process !== "undefined" && (process as any).resourcesPath
      ? path.join((process as any).resourcesPath, "control-ui")
      : null,
