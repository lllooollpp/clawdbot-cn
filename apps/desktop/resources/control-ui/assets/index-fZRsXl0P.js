...
`);const s=El(e);if(!s)return null;const a=[...s.matchAll(/<\s*think(?:ing)?\s*>([\s\S]*?)<\s*\/\s*think(?:ing)?\s*>/gi)].map(l=>(l[1]??"").trim()).filter(Boolean);return a.length>0?a.join(`
`):null}function Tl(e){if(!e||typeof e!="object")return Zs(e);const t=e;if(Un.has(t))return Un.get(t)??null;const n=Zs(e);return Un.set(t,n),n}function El(e){const t=e,n=t.content;if(typeof n=="string")return n;if(Array.isArray(n)){const i=n.map(s=>{const r=s;return r.type==="text"&&typeof r.text=="string"?r.text:null}).filter(s=>typeof s=="string");if(i.length>0)return i.join(`
`)}return typeof t.text=="string"?t.text:null}function Cl(e){const t=e.trim();if(!t)return"";const n=t.split(/\r?\n/).map(i=>i.trim()).filter(Boolean).map(i=>`_${i}_`);return n.length?["_Reasoning:_",...n].join(`
`):""}function Ys(e){e[6]=e[6]&15|64,e[8]=e[8]&63|128;let t="";for(let n=0;n<e.length;n++)t+=e[n].toString(16).padStart(2,"0");return`${t.slice(0,8)}-${t.slice(8,12)}-${t.slice(12,16)}-${t.slice(16,20)}-${t.slice(20)}`}function Ml(){const e=new Uint8Array(16),t=Date.now();for(let n=0;n<e.length;n++)e[n]=Math.floor(Math.random()*256);return e[0]^=t&255,e[1]^=t>>>8&255,e[2]^=t>>>16&255,e[3]^=t>>>24&255,e}function Di(e=globalThis.crypto){if(e&&typeof e.randomUUID=="function")return e.randomUUID();if(e&&typeof e.getRandomValues=="function"){const t=new Uint8Array(16);return e.getRandomValues(t),Ys(t)}return Ys(Ml())}async function tt(e){if(!(!e.client||!e.connected)){e.chatLoading=!0,e.lastError=null;try{const t=await e.client.request("chat.history",{sessionKey:e.sessionKey,limit:200});e.chatMessages=Array.isArray(t.messages)?t.messages:[],e.chatThinkingLevel=t.thinkingLevel??null}catch(t){e.lastError=String(t)}finally{e.chatLoading=!1}}}function Il(e){const t=/^data:([^;]+);base64,(.+)$/.exec(e);return t?{mimeType:t[1],content:t[2]}:null}async function Ll(e,t,n){if(!e.client||!e.connected)return!1;const i=t.trim(),s=n&&n.length>0;if(!i&&!s)return!1;const r=Date.now(),a=[];if(i&&a.push({type:"text",text:i}),s)for(const p of n)a.push({type:"image",source:{type:"base64",media_type:p.mimeType,data:p.dataUrl}});e.chatMessages=[...e.chatMessages,{role:"user",content:a,timestamp:r}],e.chatSending=!0,e.lastError=null;const l=Di();e.chatRunId=l,e.chatStream="",e.chatStreamStartedAt=r;const c=s?n.map(p=>{const d=Il(p.dataUrl);return d?{type:"image",mimeType:d.mimeType,content:d.content}:null}).filter(p=>p!==null):void 0;try{return await e.client.request("chat.send",{sessionKey:e.sessionKey,message:i,deliver:!1,idempotencyKey:l,attachments:c}),!0}catch(p){const d=String(p);return e.chatRunId=null,e.chatStream=null,e.chatStreamStartedAt=null,e.lastError=d,e.chatMessages=[...e.chatMessages,{role:"assistant",content:[{type:"text",text:"Error: "+d}],timestamp:Date.now()}],!1}finally{e.chatSending=!1}}async function Rl(e){if(!e.client||!e.connected)return!1;const t=e.chatRunId;try{return await e.client.request("chat.abort",t?{sessionKey:e.sessionKey,runId:t}:{sessionKey:e.sessionKey}),!0}catch(n){return e.lastError=String(n),!1}}function Pl(e,t){if(!t||t.sessionKey!==e.sessionKey)return null;if(t.runId&&e.chatRunId&&t.runId!==e.chatRunId)return t.state==="final"?"final":null;if(t.state==="delta"){const n=li(t.message);if(typeof n=="string"){const i=e.chatStream??"";(!i||n.length>=i.length)&&(e.chatStream=n)}}else t.state==="final"||t.state==="aborted"?(e.chatStream=null,e.chatRunId=null,e.chatStreamStartedAt=null):t.state==="error"&&(e.chatStream=null,e.chatRunId=null,e.chatStreamStartedAt=null,e.lastError=t.errorMessage??"chat error");return t.state}async function at(e){if(!(!e.client||!e.connected)&&!e.sessionsLoading){e.sessionsLoading=!0,e.sessionsError=null;try{const t={includeGlobal:e.sessionsIncludeGlobal,includeUnknown:e.sessionsIncludeUnknown},n=tn(e.sessionsFilterActive,0),i=tn(e.sessionsFilterLimit,0);n>0&&(t.activeMinutes=n),i>0&&(t.limit=i);const s=await e.client.request("sessions.list",t);s&&(e.sessionsResult=s)}catch(t){e.sessionsError=String(t)}finally{e.sessionsLoading=!1}}}async function Ol(e,t,n){if(!e.client||!e.connected)return;const i={key:t};"label"in n&&(i.label=n.label),"thinkingLevel"in n&&(i.thinkingLevel=n.thinkingLevel),"verboseLevel"in n&&(i.verboseLevel=n.verboseLevel),"reasoningLevel"in n&&(i.reasoningLevel=n.reasoningLevel);try{await e.client.request("sessions.patch",i),await at(e)}catch(s){e.sessionsError=String(s)}}async function Nl(e,t){if(!(!e.client||!e.connected||e.sessionsLoading||!window.confirm(`Delete session "${t}"?

Deletes the session entry and archives its transcript.`))){e.sessionsLoading=!0,e.sessionsError=null;try{await e.client.request("sessions.delete",{key:t,deleteTranscript:!0}),await at(e)}catch(i){e.sessionsError=String(i)}finally{e.sessionsLoading=!1}}}const Js=50,Dl=80,Bl=12e4;function Fl(e){if(!e||typeof e!="object")return null;const t=e;if(typeof t.text=="string")return t.text;const n=t.content;if(!Array.isArray(n))return null;const i=n.map(s=>{if(!s||typeof s!="object")return null;const r=s;return r.type==="text"&&typeof r.text=="string"?r.text:null}).filter(s=>!!s);return i.length===0?null:i.join(`
`)}function Xs(e){if(e==null)return null;if(typeof e=="number"||typeof e=="boolean")return String(e);const t=Fl(e);let n;if(typeof e=="string")n=e;else if(t)n=t;else try{n=JSON.stringify(e,null,2)}catch{n=String(e)}const i=la(n,Bl);return i.truncated?`${i.text}

… truncated (${i.total} chars, showing first ${i.text.length}).`:i.text}function zl(e){const t=[];return t.push({type:"toolcall",name:e.name,arguments:e.args??{}}),e.output&&t.push({type:"toolresult",name:e.name,text:e.output}),{role:"assistant",toolCallId:e.toolCallId,runId:e.runId,content:t,timestamp:e.startedAt}}function Ul(e){if(e.toolStreamOrder.length<=Js)return;const t=e.toolStreamOrder.length-Js,n=e.toolStreamOrder.splice(0,t);for(const i of n)e.toolStreamById.delete(i)}function Kl(e){e.chatToolMessages=e.toolStreamOrder.map(t=>e.toolStreamById.get(t)?.message).filter(t=>!!t)}function ci(e){e.toolStreamSyncTimer!=null&&(clearTimeout(e.toolStreamSyncTimer),e.toolStreamSyncTimer=null),Kl(e)}function Hl(e,t=!1){if(t){ci(e);return}e.toolStreamSyncTimer==null&&(e.toolStreamSyncTimer=window.setTimeout(()=>ci(e),Dl))}function Bi(e){e.toolStreamById.clear(),e.toolStreamOrder=[],e.chatToolMessages=[],ci(e)}const Wl=5e3;function jl(e,t){const n=t.data??{},i=typeof n.phase=="string"?n.phase:"";e.compactionClearTimer!=null&&(window.clearTimeout(e.compactionClearTimer),e.compactionClearTimer=null),i==="start"?e.compactionStatus={active:!0,startedAt:Date.now(),completedAt:null}:i==="end"&&(e.compactionStatus={active:!1,startedAt:e.compactionStatus?.startedAt??null,completedAt:Date.now()},e.compactionClearTimer=window.setTimeout(()=>{e.compactionStatus=null,e.compactionClearTimer=null},Wl))}function ql(e,t){if(!t)return;if(t.stream==="compaction"){jl(e,t);return}if(t.stream!=="tool")return;const n=typeof t.sessionKey=="string"?t.sessionKey:void 0;if(n&&n!==e.sessionKey||!n&&e.chatRunId&&t.runId!==e.chatRunId||e.chatRunId&&t.runId!==e.chatRunId||!e.chatRunId)return;const i=t.data??{},s=typeof i.toolCallId=="string"?i.toolCallId:"";if(!s)return;const r=typeof i.name=="string"?i.name:"tool",a=typeof i.phase=="string"?i.phase:"",l=a==="start"?i.args:void 0,c=a==="update"?Xs(i.partialResult):a==="result"?Xs(i.result):void 0,p=Date.now();let d=e.toolStreamById.get(s);d?(d.name=r,l!==void 0&&(d.args=l),c!==void 0&&(d.output=c),d.updatedAt=p):(d={toolCallId:s,runId:t.runId,sessionKey:n,name:r,args:l,output:c,startedAt:typeof t.ts=="number"?t.ts:p,updatedAt:p,message:{}},e.toolStreamById.set(s,d),e.toolStreamOrder.push(s)),d.message=zl(d),Ul(e),Hl(e,a==="result")}function fn(e,t=!1){e.chatScrollFrame&&cancelAnimationFrame(e.chatScrollFrame),e.chatScrollTimeout!=null&&(clearTimeout(e.chatScrollTimeout),e.chatScrollTimeout=null);const n=()=>{const i=e.querySelector(".chat-thread");if(i){const s=getComputedStyle(i).overflowY;if(s==="auto"||s==="scroll"||i.scrollHeight-i.clientHeight>1)return i}return document.scrollingElement??document.documentElement};e.updateComplete.then(()=>{e.chatScrollFrame=requestAnimationFrame(()=>{e.chatScrollFrame=null;const i=n();if(!i)return;const s=i.scrollHeight-i.scrollTop-i.clientHeight;if(!(t||e.chatUserNearBottom||s<200))return;t&&(e.chatHasAutoScrolled=!0),i.scrollTop=i.scrollHeight,e.chatUserNearBottom=!0;const a=t?150:120;e.chatScrollTimeout=window.setTimeout(()=>{e.chatScrollTimeout=null;const l=n();if(!l)return;const c=l.scrollHeight-l.scrollTop-l.clientHeight;(t||e.chatUserNearBottom||c<200)&&(l.scrollTop=l.scrollHeight,e.chatUserNearBottom=!0)},a)})})}function da(e,t=!1){e.logsScrollFrame&&cancelAnimationFrame(e.logsScrollFrame),e.updateComplete.then(()=>{e.logsScrollFrame=requestAnimationFrame(()=>{e.logsScrollFrame=null;const n=e.querySelector(".log-stream");if(!n)return;const i=n.scrollHeight-n.scrollTop-n.clientHeight;(t||i<80)&&(n.scrollTop=n.scrollHeight)})})}function Vl(e,t){const n=t.currentTarget;if(!n)return;const i=n.scrollHeight-n.scrollTop-n.clientHeight;e.chatUserNearBottom=i<200}function Gl(e,t){const n=t.currentTarget;if(!n)return;const i=n.scrollHeight-n.scrollTop-n.clientHeight;e.logsAtBottom=i<80}function Ql(e){e.chatHasAutoScrolled=!1,e.chatUserNearBottom=!0}function Zl(e,t){if(e.length===0)return;const n=new Blob([`${e.join(`
`)}
`],{type:"text/plain"}),i=URL.createObjectURL(n),s=document.createElement("a"),r=new Date().toISOString().slice(0,19).replace(/[:T]/g,"-");s.href=i,s.download=`clawdbot-logs-${t}-${r}.log`,s.click(),URL.revokeObjectURL(i)}function Yl(e){if(typeof ResizeObserver>"u")return;const t=e.querySelector(".topbar");if(!t)return;const n=()=>{const{height:i}=t.getBoundingClientRect();e.style.setProperty("--topbar-height",`${i}px`)};n(),e.topbarObserver=new ResizeObserver(()=>n()),e.topbarObserver.observe(t)}function Be(e){return typeof structuredClone=="function"?structuredClone(e):JSON.parse(JSON.stringify(e))}function nt(e){return`${JSON.stringify(e,null,2).trimEnd()}
`}function ua(e,t,n){if(t.length===0)return;let i=e;for(let r=0;r<t.length-1;r+=1){const a=t[r],l=t[r+1];if(typeof a=="number"){if(!Array.isArray(i))return;i[a]==null&&(i[a]=typeof l=="number"?[]:{}),i=i[a]}else{if(typeof i!="object"||i==null)return;const c=i;c[a]==null&&(c[a]=typeof l=="number"?[]:{}),i=c[a]}}const s=t[t.length-1];if(typeof s=="number"){Array.isArray(i)&&(i[s]=n);return}typeof i=="object"&&i!=null&&(i[s]=n)}function pa(e,t){if(t.length===0)return;let n=e;for(let s=0;s<t.length-1;s+=1){const r=t[s];if(typeof r=="number"){if(!Array.isArray(n))return;n=n[r]}else{if(typeof n!="object"||n==null)return;n=n[r]}if(n==null)return}const i=t[t.length-1];if(typeof i=="number"){Array.isArray(n)&&n.splice(i,1);return}typeof n=="object"&&n!=null&&delete n[i]}async function ye(e){if(!(!e.client||!e.connected)){e.configLoading=!0,e.lastError=null;try{const t=await e.client.request("config.get",{});Xl(e,t)}catch(t){e.lastError=String(t)}finally{e.configLoading=!1}}}async function fa(e){if(!(!e.client||!e.connected)&&!e.configSchemaLoading){e.configSchemaLoading=!0;try{const t=await e.client.request("config.schema",{});Jl(e,t)}catch(t){e.lastError=String(t)}finally{e.configSchemaLoading=!1}}}function Jl(e,t){e.configSchema=t.schema??null,e.configUiHints=t.uiHints??{},e.configSchemaVersion=t.version??null}function Xl(e,t){e.configSnapshot=t;const n=typeof t.raw=="string"?t.raw:t.config&&typeof t.config=="object"?nt(t.config):e.configRaw;!e.configFormDirty||e.configFormMode==="raw"?e.configRaw=n:e.configForm?e.configRaw=nt(e.configForm):e.configRaw=n,e.configValid=typeof t.valid=="boolean"?t.valid:null,e.configIssues=Array.isArray(t.issues)?t.issues:[],e.configFormDirty||(e.configForm=Be(t.config??{}),e.configFormOriginal=Be(t.config??{}),e.configRawOriginal=n)}async function di(e){if(!(!e.client||!e.connected)){e.configSaving=!0,e.lastError=null;try{const t=e.configFormMode==="form"&&e.configForm?nt(e.configForm):e.configRaw,n=e.configSnapshot?.hash;if(!n){e.lastError="Config hash missing; reload and retry.";return}await e.client.request("config.set",{raw:t,baseHash:n}),e.configFormDirty=!1,await ye(e)}catch(t){e.lastError=String(t)}finally{e.configSaving=!1}}}async function ec(e){if(!(!e.client||!e.connected)){e.configApplying=!0,e.lastError=null;try{const t=e.configFormMode==="form"&&e.configForm?nt(e.configForm):e.configRaw,n=e.configSnapshot?.hash;if(!n){e.lastError="Config hash missing; reload and retry.";return}await e.client.request("config.apply",{raw:t,baseHash:n,sessionKey:e.applySessionKey}),e.configFormDirty=!1,await ye(e)}catch(t){e.lastError=String(t)}finally{e.configApplying=!1}}}async function tc(e){if(!(!e.client||!e.connected)){e.updateRunning=!0,e.lastError=null;try{await e.client.request("update.run",{sessionKey:e.applySessionKey})}catch(t){e.lastError=String(t)}finally{e.updateRunning=!1}}}function yt(e,t,n){const i=Be(e.configForm??e.configSnapshot?.config??{});ua(i,t,n),e.configForm=i,e.configFormDirty=!0,e.configFormMode==="form"&&(e.configRaw=nt(i))}function er(e,t){const n=Be(e.configForm??e.configSnapshot?.config??{});pa(n,t),e.configForm=n,e.configFormDirty=!0,e.configFormMode==="form"&&(e.configRaw=nt(n))}async function nc(e,t){if(!e.client||!e.connected||!e.configForm)return;const n=e.configForm.models;if(!n||!n.providers)return;const i=n.providers[t];if(i){e.configSaving=!0,e.lastError=null;try{const s=await e.client.request("models.discover",{provider:t,apiKey:i.apiKey,baseUrl:i.baseUrl});if(s&&Array.isArray(s.models)&&s.models.length>0){const r=i.models??[],a=s.models;return yt(e,["models","providers",t,"models"],a),{success:!0,count:a.length}}else return e.lastError="No models found or error during discovery.",{success:!1,error:e.lastError}}catch(s){return e.lastError=String(s),{success:!1,error:e.lastError}}finally{e.configSaving=!1}}}async function It(e){if(!(!e.client||!e.connected))try{const t=await e.client.request("cron.status",{});e.cronStatus=t}catch(t){e.cronError=String(t)}}async function hn(e){if(!(!e.client||!e.connected)&&!e.cronLoading){e.cronLoading=!0,e.cronError=null;try{const t=await e.client.request("cron.list",{includeDisabled:!0});e.cronJobs=Array.isArray(t.jobs)?t.jobs:[]}catch(t){e.cronError=String(t)}finally{e.cronLoading=!1}}}function ic(e){if(e.scheduleKind==="at"){const n=Date.parse(e.scheduleAt);if(!Number.isFinite(n))throw new Error("Invalid run time.");return{kind:"at",atMs:n}}if(e.scheduleKind==="every"){const n=tn(e.everyAmount,0);if(n<=0)throw new Error("Invalid interval amount.");const i=e.everyUnit;return{kind:"every",everyMs:n*(i==="minutes"?6e4:i==="hours"?36e5:864e5)}}const t=e.cronExpr.trim();if(!t)throw new Error("Cron expression required.");return{kind:"cron",expr:t,tz:e.cronTz.trim()||void 0}}function sc(e){if(e.payloadKind==="systemEvent"){const s=e.payloadText.trim();if(!s)throw new Error("System event text required.");return{kind:"systemEvent",text:s}}const t=e.payloadText.trim();if(!t)throw new Error("Agent message required.");const n={kind:"agentTurn",message:t};e.deliver&&(n.deliver=!0),e.channel&&(n.channel=e.channel),e.to.trim()&&(n.to=e.to.trim());const i=tn(e.timeoutSeconds,0);return i>0&&(n.timeoutSeconds=i),n}async function rc(e){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{const t=ic(e.cronForm),n=sc(e.cronForm),i=e.cronForm.agentId.trim(),s={name:e.cronForm.name.trim(),description:e.cronForm.description.trim()||void 0,agentId:i||void 0,enabled:e.cronForm.enabled,schedule:t,sessionTarget:e.cronForm.sessionTarget,wakeMode:e.cronForm.wakeMode,payload:n,isolation:e.cronForm.postToMainPrefix.trim()&&e.cronForm.sessionTarget==="isolated"?{postToMainPrefix:e.cronForm.postToMainPrefix.trim()}:void 0};if(!s.name)throw new Error("Name required.");await e.client.request("cron.add",s),e.cronForm={...e.cronForm,name:"",description:"",payloadText:""},await hn(e),await It(e)}catch(t){e.cronError=String(t)}finally{e.cronBusy=!1}}}async function ac(e,t,n){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.update",{id:t.id,patch:{enabled:n}}),await hn(e),await It(e)}catch(i){e.cronError=String(i)}finally{e.cronBusy=!1}}}async function oc(e,t){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.run",{id:t.id,mode:"force"}),await ha(e,t.id)}catch(n){e.cronError=String(n)}finally{e.cronBusy=!1}}}async function lc(e,t){if(!(!e.client||!e.connected||e.cronBusy)){e.cronBusy=!0,e.cronError=null;try{await e.client.request("cron.remove",{id:t.id}),e.cronRunsJobId===t.id&&(e.cronRunsJobId=null,e.cronRuns=[]),await hn(e),await It(e)}catch(n){e.cronError=String(n)}finally{e.cronBusy=!1}}}async function ha(e,t){if(!(!e.client||!e.connected))try{const n=await e.client.request("cron.runs",{id:t,limit:50});e.cronRunsJobId=t,e.cronRuns=Array.isArray(n.entries)?n.entries:[]}catch(n){e.cronError=String(n)}}async function le(e,t){if(!(!e.client||!e.connected)&&!e.channelsLoading){e.channelsLoading=!0,e.channelsError=null;try{const n=await e.client.request("channels.status",{probe:t,timeoutMs:8e3});e.channelsSnapshot=n,e.channelsLastSuccess=Date.now()}catch(n){e.channelsError=String(n)}finally{e.channelsLoading=!1}}}async function cc(e,t){if(!(!e.client||!e.connected||e.whatsappBusy)){e.whatsappBusy=!0;try{const n=await e.client.request("web.login.start",{force:t,timeoutMs:3e4});e.whatsappLoginMessage=n.message??null,e.whatsappLoginQrDataUrl=n.qrDataUrl??null,e.whatsappLoginConnected=null}catch(n){e.whatsappLoginMessage=String(n),e.whatsappLoginQrDataUrl=null,e.whatsappLoginConnected=null}finally{e.whatsappBusy=!1}}}async function dc(e){if(!(!e.client||!e.connected||e.whatsappBusy)){e.whatsappBusy=!0;try{const t=await e.client.request("web.login.wait",{timeoutMs:12e4});e.whatsappLoginMessage=t.message??null,e.whatsappLoginConnected=t.connected??null,t.connected&&(e.whatsappLoginQrDataUrl=null)}catch(t){e.whatsappLoginMessage=String(t),e.whatsappLoginConnected=null}finally{e.whatsappBusy=!1}}}async function uc(e){if(!(!e.client||!e.connected||e.whatsappBusy)){e.whatsappBusy=!0;try{await e.client.request("channels.logout",{channel:"whatsapp"}),e.whatsappLoginMessage="Logged out.",e.whatsappLoginQrDataUrl=null,e.whatsappLoginConnected=null}catch(t){e.whatsappLoginMessage=String(t)}finally{e.whatsappBusy=!1}}}async function gn(e){if(!(!e.client||!e.connected)&&!e.debugLoading){e.debugLoading=!0;try{const[t,n,i,s]=await Promise.all([e.client.request("status",{}),e.client.request("health",{}),e.client.request("models.list",{}),e.client.request("last-heartbeat",{})]);e.debugStatus=t,e.debugHealth=n;const r=i;e.debugModels=Array.isArray(r?.models)?r?.models:[],e.debugHeartbeat=s}catch(t){e.debugCallError=String(t)}finally{e.debugLoading=!1}}}async function pc(e){if(!(!e.client||!e.connected)){e.debugCallError=null,e.debugCallResult=null;try{const t=e.debugCallParams.trim()?JSON.parse(e.debugCallParams):{},n=await e.client.request(e.debugCallMethod.trim(),t);e.debugCallResult=JSON.stringify(n,null,2)}catch(t){e.debugCallError=String(t)}}}const fc=2e3,hc=new Set(["trace","debug","info","warn","error","fatal"]);function gc(e){if(typeof e!="string")return null;const t=e.trim();if(!t.startsWith("{")||!t.endsWith("}"))return null;try{const n=JSON.parse(t);return!n||typeof n!="object"?null:n}catch{return null}}function vc(e){if(typeof e!="string")return null;const t=e.toLowerCase();return hc.has(t)?t:null}function mc(e){if(!e.trim())return{raw:e,message:e};try{const t=JSON.parse(e),n=t&&typeof t._meta=="object"&&t._meta!==null?t._meta:null,i=typeof t.time=="string"?t.time:typeof n?.date=="string"?n?.date:null,s=vc(n?.logLevelName??n?.level),r=typeof t[0]=="string"?t[0]:typeof n?.name=="string"?n?.name:null,a=gc(r);let l=null;a&&(typeof a.subsystem=="string"?l=a.subsystem:typeof a.module=="string"&&(l=a.module)),!l&&r&&r.length<120&&(l=r);let c=null;return typeof t[1]=="string"?c=t[1]:!a&&typeof t[0]=="string"?c=t[0]:typeof t.message=="string"&&(c=t.message),{raw:e,time:i,level:s,subsystem:l,message:c??e,meta:n??void 0}}catch{return{raw:e,message:e}}}async function Fi(e,t){if(!(!e.client||!e.connected)&&!(e.logsLoading&&!t?.quiet)){t?.quiet||(e.logsLoading=!0),e.logsError=null;try{const i=await e.client.request("logs.tail",{cursor:t?.reset?void 0:e.logsCursor??void 0,limit:e.logsLimit,maxBytes:e.logsMaxBytes}),r=(Array.isArray(i.lines)?i.lines.filter(l=>typeof l=="string"):[]).map(mc),a=!!(t?.reset||i.reset||e.logsCursor==null);e.logsEntries=a?r:[...e.logsEntries,...r].slice(-fc),typeof i.cursor=="number"&&(e.logsCursor=i.cursor),typeof i.file=="string"&&(e.logsFile=i.file),e.logsTruncated=!!i.truncated,e.logsLastFetchAt=Date.now()}catch(n){e.logsError=String(n)}finally{t?.quiet||(e.logsLoading=!1)}}}const ga={p:0x7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffedn,n:0x1000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3edn,h:8n,a:0x7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffecn,d:0x52036cee2b6ffe738cc740797779e89800700a4d4141d8ab75eb4dca135978a3n,Gx:0x216936d3cd6e53fec0a4e231fdd6dc5c692cc7609525a7b2c9562d608f25d51an,Gy:0x6666666666666666666666666666666666666666666666666666666666666658n},{p:V,n:Yt,Gx:tr,Gy:nr,a:Hn,d:Wn,h:bc}=ga,Fe=32,zi=64,yc=(...e)=>{"captureStackTrace"in Error&&typeof Error.captureStackTrace=="function"&&Error.captureStackTrace(...e)},H=(e="")=>{const t=new Error(e);throw yc(t,H),t},$c=e=>typeof e=="bigint",wc=e=>typeof e=="string",xc=e=>e instanceof Uint8Array||ArrayBuffer.isView(e)&&e.constructor.name==="Uint8Array",Te=(e,t,n="")=>{const i=xc(e),s=e?.length,r=t!==void 0;if(!i||r&&s!==t){const a=n&&`"${n}" `,l=r?` of length ${t}`:"",c=i?`length=${s}`:`type=${typeof e}`;H(a+"expected Uint8Array"+l+", got "+c)}return e},vn=e=>new Uint8Array(e),va=e=>Uint8Array.from(e),ma=(e,t)=>e.toString(16).padStart(t,"0"),ba=e=>Array.from(Te(e)).map(t=>ma(t,2)).join(""),me={_0:48,_9:57,A:65,F:70,a:97,f:102},ir=e=>{if(e>=me._0&&e<=me._9)return e-me._0;if(e>=me.A&&e<=me.F)return e-(me.A-10);if(e>=me.a&&e<=me.f)return e-(me.a-10)},ya=e=>{const t="hex invalid";if(!wc(e))return H(t);const n=e.length,i=n/2;if(n%2)return H(t);const s=vn(i);for(let r=0,a=0;r<i;r++,a+=2){const l=ir(e.charCodeAt(a)),c=ir(e.charCodeAt(a+1));if(l===void 0||c===void 0)return H(t);s[r]=l*16+c}return s},$a=()=>globalThis?.crypto,kc=()=>$a()?.subtle??H("crypto.subtle must be defined, consider polyfill"),Ct=(...e)=>{const t=vn(e.reduce((i,s)=>i+Te(s).length,0));let n=0;return e.forEach(i=>{t.set(i,n),n+=i.length}),t},Ac=(e=Fe)=>$a().getRandomValues(vn(e)),nn=BigInt,Pe=(e,t,n,i="bad number: out of range")=>$c(e)&&t<=e&&e<n?e:H(i),S=(e,t=V)=>{const n=e%t;return n>=0n?n:t+n},wa=e=>S(e,Yt),Sc=(e,t)=>{(e===0n||t<=0n)&&H("no inverse n="+e+" mod="+t);let n=S(e,t),i=t,s=0n,r=1n;for(;n!==0n;){const a=i/n,l=i%n,c=s-r*a;i=n,n=l,s=r,r=c}return i===1n?S(s,t):H("no inverse")},_c=e=>{const t=Sa[e];return typeof t!="function"&&H("hashes."+e+" not set"),t},jn=e=>e instanceof ne?e:H("Point expected"),ui=2n**256n;class ne{static BASE;static ZERO;X;Y;Z;T;constructor(t,n,i,s){const r=ui;this.X=Pe(t,0n,r),this.Y=Pe(n,0n,r),this.Z=Pe(i,1n,r),this.T=Pe(s,0n,r),Object.freeze(this)}static CURVE(){return ga}static fromAffine(t){return new ne(t.x,t.y,1n,S(t.x*t.y))}static fromBytes(t,n=!1){const i=Wn,s=va(Te(t,Fe)),r=t[31];s[31]=r&-129;const a=ka(s);Pe(a,0n,n?ui:V);const c=S(a*a),p=S(c-1n),d=S(i*c+1n);let{isValid:u,value:v}=Ec(p,d);u||H("bad point: y not sqrt");const g=(v&1n)===1n,$=(r&128)!==0;return!n&&v===0n&&$&&H("bad point: x==0, isLastByteOdd"),$!==g&&(v=S(-v)),new ne(v,a,1n,S(v*a))}static fromHex(t,n){return ne.fromBytes(ya(t),n)}get x(){return this.toAffine().x}get y(){return this.toAffine().y}assertValidity(){const t=Hn,n=Wn,i=this;if(i.is0())return H("bad point: ZERO");const{X:s,Y:r,Z:a,T:l}=i,c=S(s*s),p=S(r*r),d=S(a*a),u=S(d*d),v=S(c*t),g=S(d*S(v+p)),$=S(u+S(n*S(c*p)));if(g!==$)return H("bad point: equation left != right (1)");const w=S(s*r),k=S(a*l);return w!==k?H("bad point: equation left != right (2)"):this}equals(t){const{X:n,Y:i,Z:s}=this,{X:r,Y:a,Z:l}=jn(t),c=S(n*l),p=S(r*s),d=S(i*l),u=S(a*s);return c===p&&d===u}is0(){return this.equals(Je)}negate(){return new ne(S(-this.X),this.Y,this.Z,S(-this.T))}double(){const{X:t,Y:n,Z:i}=this,s=Hn,r=S(t*t),a=S(n*n),l=S(2n*S(i*i)),c=S(s*r),p=t+n,d=S(S(p*p)-r-a),u=c+a,v=u-l,g=c-a,$=S(d*v),w=S(u*g),k=S(d*g),E=S(v*u);return new ne($,w,E,k)}add(t){const{X:n,Y:i,Z:s,T:r}=this,{X:a,Y:l,Z:c,T:p}=jn(t),d=Hn,u=Wn,v=S(n*a),g=S(i*l),$=S(r*u*p),w=S(s*c),k=S((n+i)*(a+l)-v-g),E=S(w-$),C=S(w+$),N=S(g-d*v),I=S(k*E),L=S(C*N),A=S(k*N),O=S(E*C);return new ne(I,L,O,A)}subtract(t){return this.add(jn(t).negate())}multiply(t,n=!0){if(!n&&(t===0n||this.is0()))return Je;if(Pe(t,1n,Yt),t===1n)return this;if(this.equals(ze))return Fc(t).p;let i=Je,s=ze;for(let r=this;t>0n;r=r.double(),t>>=1n)t&1n?i=i.add(r):n&&(s=s.add(r));return i}multiplyUnsafe(t){return this.multiply(t,!1)}toAffine(){const{X:t,Y:n,Z:i}=this;if(this.equals(Je))return{x:0n,y:1n};const s=Sc(i,V);S(i*s)!==1n&&H("invalid inverse");const r=S(t*s),a=S(n*s);return{x:r,y:a}}toBytes(){const{x:t,y:n}=this.assertValidity().toAffine(),i=xa(n);return i[31]|=t&1n?128:0,i}toHex(){return ba(this.toBytes())}clearCofactor(){return this.multiply(nn(bc),!1)}isSmallOrder(){return this.clearCofactor().is0()}isTorsionFree(){let t=this.multiply(Yt/2n,!1).double();return Yt%2n&&(t=t.add(this)),t.is0()}}const ze=new ne(tr,nr,1n,S(tr*nr)),Je=new ne(0n,1n,1n,0n);ne.BASE=ze;ne.ZERO=Je;const xa=e=>ya(ma(Pe(e,0n,ui),zi)).reverse(),ka=e=>nn("0x"+ba(va(Te(e)).reverse())),ue=(e,t)=>{let n=e;for(;t-- >0n;)n*=n,n%=V;return n},Tc=e=>{const n=e*e%V*e%V,i=ue(n,2n)*n%V,s=ue(i,1n)*e%V,r=ue(s,5n)*s%V,a=ue(r,10n)*r%V,l=ue(a,20n)*a%V,c=ue(l,40n)*l%V,p=ue(c,80n)*c%V,d=ue(p,80n)*c%V,u=ue(d,10n)*r%V;return{pow_p_5_8:ue(u,2n)*e%V,b2:n}},sr=0x2b8324804fc1df0b2b4d00993dfbd7a72f431806ad2fe478c4ee1b274a0ea0b0n,Ec=(e,t)=>{const n=S(t*t*t),i=S(n*n*t),s=Tc(e*i).pow_p_5_8;let r=S(e*n*s);const a=S(t*r*r),l=r,c=S(r*sr),p=a===e,d=a===S(-e),u=a===S(-e*sr);return p&&(r=l),(d||u)&&(r=c),(S(r)&1n)===1n&&(r=S(-r)),{isValid:p||d,value:r}},pi=e=>wa(ka(e)),Ui=(...e)=>Sa.sha512Async(Ct(...e)),Cc=(...e)=>_c("sha512")(Ct(...e)),Aa=e=>{const t=e.slice(0,Fe);t[0]&=248,t[31]&=127,t[31]|=64;const n=e.slice(Fe,zi),i=pi(t),s=ze.multiply(i),r=s.toBytes();return{head:t,prefix:n,scalar:i,point:s,pointBytes:r}},Ki=e=>Ui(Te(e,Fe)).then(Aa),Mc=e=>Aa(Cc(Te(e,Fe))),Ic=e=>Ki(e).then(t=>t.pointBytes),Lc=e=>Ui(e.hashable).then(e.finish),Rc=(e,t,n)=>{const{pointBytes:i,scalar:s}=e,r=pi(t),a=ze.multiply(r).toBytes();return{hashable:Ct(a,i,n),finish:p=>{const d=wa(r+pi(p)*s);return Te(Ct(a,xa(d)),zi)}}},Pc=async(e,t)=>{const n=Te(e),i=await Ki(t),s=await Ui(i.prefix,n);return Lc(Rc(i,s,n))},Sa={sha512Async:async e=>{const t=kc(),n=Ct(e);return vn(await t.digest("SHA-512",n.buffer))},sha512:void 0},Oc=(e=Ac(Fe))=>e,Nc={getExtendedPublicKeyAsync:Ki,getExtendedPublicKey:Mc,randomSecretKey:Oc},sn=8,Dc=256,_a=Math.ceil(Dc/sn)+1,fi=2**(sn-1),Bc=()=>{const e=[];let t=ze,n=t;for(let i=0;i<_a;i++){n=t,e.push(n);for(let s=1;s<fi;s++)n=n.add(t),e.push(n);t=n.double()}return e};let rr;const ar=(e,t)=>{const n=t.negate();return e?n:t},Fc=e=>{const t=rr||(rr=Bc());let n=Je,i=ze;const s=2**sn,r=s,a=nn(s-1),l=nn(sn);for(let c=0;c<_a;c++){let p=Number(e&a);e>>=l,p>fi&&(p-=r,e+=1n);const d=c*fi,u=d,v=d+Math.abs(p)-1,g=c%2!==0,$=p<0;p===0?i=i.add(ar(g,t[u])):n=n.add(ar($,t[v]))}return e!==0n&&H("invalid wnaf"),{p:n,f:i}},qn="clawdbot-device-identity-v1";function hi(e){let t="";for(const n of e)t+=String.fromCharCode(n);return btoa(t).replaceAll("+","-").replaceAll("/","_").replace(/=+$/g,"")}function Ta(e){const t=e.replaceAll("-","+").replaceAll("_","/"),n=t+"=".repeat((4-t.length%4)%4),i=atob(n),s=new Uint8Array(i.length);for(let r=0;r<i.length;r+=1)s[r]=i.charCodeAt(r);return s}function zc(e){return Array.from(e).map(t=>t.toString(16).padStart(2,"0")).join("")}async function Ea(e){const t=await crypto.subtle.digest("SHA-256",e);return zc(new Uint8Array(t))}async function Uc(){const e=Nc.randomSecretKey(),t=await Ic(e);return{deviceId:await Ea(t),publicKey:hi(t),privateKey:hi(e)}}async function Hi(){try{const n=localStorage.getItem(qn);if(n){const i=JSON.parse(n);if(i?.version===1&&typeof i.deviceId=="string"&&typeof i.publicKey=="string"&&typeof i.privateKey=="string"){const s=await Ea(Ta(i.publicKey));if(s!==i.deviceId){const r={...i,deviceId:s};return localStorage.setItem(qn,JSON.stringify(r)),{deviceId:s,publicKey:i.publicKey,privateKey:i.privateKey}}return{deviceId:i.deviceId,publicKey:i.publicKey,privateKey:i.privateKey}}}}catch{}const e=await Uc(),t={version:1,deviceId:e.deviceId,publicKey:e.publicKey,privateKey:e.privateKey,createdAtMs:Date.now()};return localStorage.setItem(qn,JSON.stringify(t)),e}async function Kc(e,t){const n=Ta(e),i=new TextEncoder().encode(t),s=await Pc(i,n);return hi(s)}const Ca="clawdbot.device.auth.v1";function Wi(e){return e.trim()}function Hc(e){if(!Array.isArray(e))return[];const t=new Set;for(const n of e){const i=n.trim();i&&t.add(i)}return[...t].sort()}function ji(){try{const e=window.localStorage.getItem(Ca);if(!e)return null;const t=JSON.parse(e);return!t||t.version!==1||!t.deviceId||typeof t.deviceId!="string"||!t.tokens||typeof t.tokens!="object"?null:t}catch{return null}}function Ma(e){try{window.localStorage.setItem(Ca,JSON.stringify(e))}catch{}}function Wc(e){const t=ji();if(!t||t.deviceId!==e.deviceId)return null;const n=Wi(e.role),i=t.tokens[n];return!i||typeof i.token!="string"?null:i}function Ia(e){const t=Wi(e.role),n={version:1,deviceId:e.deviceId,tokens:{}},i=ji();i&&i.deviceId===e.deviceId&&(n.tokens={...i.tokens});const s={token:e.token,role:t,scopes:Hc(e.scopes),updatedAtMs:Date.now()};return n.tokens[t]=s,Ma(n),s}function La(e){const t=ji();if(!t||t.deviceId!==e.deviceId)return;const n=Wi(e.role);if(!t.tokens[n])return;const i={...t,tokens:{...t.tokens}};delete i.tokens[n],Ma(i)}async function Ee(e,t){if(!(!e.client||!e.connected)&&!e.devicesLoading){e.devicesLoading=!0,t?.quiet||(e.devicesError=null);try{const n=await e.client.request("device.pair.list",{});e.devicesList={pending:Array.isArray(n?.pending)?n.pending:[],paired:Array.isArray(n?.paired)?n.paired:[]}}catch(n){t?.quiet||(e.devicesError=String(n))}finally{e.devicesLoading=!1}}}async function jc(e,t){if(!(!e.client||!e.connected))try{await e.client.request("device.pair.approve",{requestId:t}),await Ee(e)}catch(n){e.devicesError=String(n)}}async function qc(e,t){if(!(!e.client||!e.connected||!window.confirm("Reject this device pairing request?")))try{await e.client.request("device.pair.reject",{requestId:t}),await Ee(e)}catch(i){e.devicesError=String(i)}}async function Vc(e,t){if(!(!e.client||!e.connected))try{const n=await e.client.request("device.token.rotate",t);if(n?.token){const i=await Hi(),s=n.role??t.role;(n.deviceId===i.deviceId||t.deviceId===i.deviceId)&&Ia({deviceId:i.deviceId,role:s,token:n.token,scopes:n.scopes??t.scopes??[]}),window.prompt("New device token (copy and store securely):",n.token)}await Ee(e)}catch(n){e.devicesError=String(n)}}async function Gc(e,t){if(!(!e.client||!e.connected||!window.confirm(`Revoke token for ${t.deviceId} (${t.role})?`)))try{await e.client.request("device.token.revoke",t);const i=await Hi();t.deviceId===i.deviceId&&La({deviceId:i.deviceId,role:t.role}),await Ee(e)}catch(i){e.devicesError=String(i)}}async function mn(e,t){if(!(!e.client||!e.connected)&&!e.nodesLoading){e.nodesLoading=!0,t?.quiet||(e.lastError=null);try{const n=await e.client.request("node.list",{});e.nodes=Array.isArray(n.nodes)?n.nodes:[]}catch(n){t?.quiet||(e.lastError=String(n))}finally{e.nodesLoading=!1}}}function Qc(e){if(!e||e.kind==="gateway")return{method:"exec.approvals.get",params:{}};const t=e.nodeId.trim();return t?{method:"exec.approvals.node.get",params:{nodeId:t}}:null}function Zc(e,t){if(!e||e.kind==="gateway")return{method:"exec.approvals.set",params:t};const n=e.nodeId.trim();return n?{method:"exec.approvals.node.set",params:{...t,nodeId:n}}:null}async function qi(e,t){if(!(!e.client||!e.connected)&&!e.execApprovalsLoading){e.execApprovalsLoading=!0,e.lastError=null;try{const n=Qc(t);if(!n){e.lastError="Select a node before loading exec approvals.";return}const i=await e.client.request(n.method,n.params);Yc(e,i)}catch(n){e.lastError=String(n)}finally{e.execApprovalsLoading=!1}}}function Yc(e,t){e.execApprovalsSnapshot=t,e.execApprovalsDirty||(e.execApprovalsForm=Be(t.file??{}))}async function Jc(e,t){if(!(!e.client||!e.connected)){e.execApprovalsSaving=!0,e.lastError=null;try{const n=e.execApprovalsSnapshot?.hash;if(!n){e.lastError="Exec approvals hash missing; reload and retry.";return}const i=e.execApprovalsForm??e.execApprovalsSnapshot?.file??{},s=Zc(t,{file:i,baseHash:n});if(!s){e.lastError="Select a node before saving exec approvals.";return}await e.client.request(s.method,s.params),e.execApprovalsDirty=!1,await qi(e,t)}catch(n){e.lastError=String(n)}finally{e.execApprovalsSaving=!1}}}function Xc(e,t,n){const i=Be(e.execApprovalsForm??e.execApprovalsSnapshot?.file??{});ua(i,t,n),e.execApprovalsForm=i,e.execApprovalsDirty=!0}function ed(e,t){const n=Be(e.execApprovalsForm??e.execApprovalsSnapshot?.file??{});pa(n,t),e.execApprovalsForm=n,e.execApprovalsDirty=!0}async function Vi(e){if(!(!e.client||!e.connected)&&!e.presenceLoading){e.presenceLoading=!0,e.presenceError=null,e.presenceStatus=null;try{const t=await e.client.request("system-presence",{});Array.isArray(t)?(e.presenceEntries=t,e.presenceStatus=t.length===0?"No instances yet.":null):(e.presenceEntries=[],e.presenceStatus="No presence payload.")}catch(t){e.presenceError=String(t)}finally{e.presenceLoading=!1}}}function it(e,t,n){if(!t.trim())return;const i={...e.skillMessages};n?i[t]=n:delete i[t],e.skillMessages=i}function bn(e){return e instanceof Error?e.message:String(e)}async function Lt(e,t){if(t?.clearMessages&&Object.keys(e.skillMessages).length>0&&(e.skillMessages={}),!(!e.client||!e.connected)&&!e.skillsLoading){e.skillsLoading=!0,e.skillsError=null;try{const n=await e.client.request("skills.status",{});n&&(e.skillsReport=n)}catch(n){e.skillsError=bn(n)}finally{e.skillsLoading=!1}}}function td(e,t,n){e.skillEdits={...e.skillEdits,[t]:n}}async function nd(e,t,n){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{await e.client.request("skills.update",{skillKey:t,enabled:n}),await Lt(e),it(e,t,{kind:"success",message:n?"Skill enabled":"Skill disabled"})}catch(i){const s=bn(i);e.skillsError=s,it(e,t,{kind:"error",message:s})}finally{e.skillsBusyKey=null}}}async function id(e,t){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{const n=e.skillEdits[t]??"";await e.client.request("skills.update",{skillKey:t,apiKey:n}),await Lt(e),it(e,t,{kind:"success",message:"API key saved"})}catch(n){const i=bn(n);e.skillsError=i,it(e,t,{kind:"error",message:i})}finally{e.skillsBusyKey=null}}}async function sd(e,t,n,i){if(!(!e.client||!e.connected)){e.skillsBusyKey=t,e.skillsError=null;try{const s=await e.client.request("skills.install",{name:n,installId:i,timeoutMs:12e4});await Lt(e),it(e,t,{kind:"success",message:s?.message??"Installed"})}catch(s){const r=bn(s);e.skillsError=r,it(e,t,{kind:"error",message:r})}finally{e.skillsBusyKey=null}}}function rd(){return typeof window>"u"||typeof window.matchMedia!="function"||window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"}function Gi(e){return e==="system"?rd():e}const Wt=e=>Number.isNaN(e)?.5:e<=0?0:e>=1?1:e,ad=()=>typeof window>"u"||typeof window.matchMedia!="function"?!1:window.matchMedia("(prefers-reduced-motion: reduce)").matches??!1,jt=e=>{e.classList.remove("theme-transition"),e.style.removeProperty("--theme-switch-x"),e.style.removeProperty("--theme-switch-y")},od=({nextTheme:e,applyTheme:t,context:n,currentTheme:i})=>{if(i===e)return;const s=globalThis.document??null;if(!s){t();return}const r=s.documentElement,a=s,l=ad();if(!!a.startViewTransition&&!l){let p=.5,d=.5;if(n?.pointerClientX!==void 0&&n?.pointerClientY!==void 0&&typeof window<"u")p=Wt(n.pointerClientX/window.innerWidth),d=Wt(n.pointerClientY/window.innerHeight);else if(n?.element){const u=n.element.getBoundingClientRect();u.width>0&&u.height>0&&typeof window<"u"&&(p=Wt((u.left+u.width/2)/window.innerWidth),d=Wt((u.top+u.height/2)/window.innerHeight))}r.style.setProperty("--theme-switch-x",`${p*100}%`),r.style.setProperty("--theme-switch-y",`${d*100}%`),r.classList.add("theme-transition");try{const u=a.startViewTransition?.(()=>{t()});u?.finished?u.finished.finally(()=>jt(r)):jt(r)}catch{jt(r),t()}return}t(),jt(r)};function ld(e){e.nodesPollInterval==null&&(e.nodesPollInterval=window.setInterval(()=>{mn(e,{quiet:!0})},5e3))}function cd(e){e.nodesPollInterval!=null&&(clearInterval(e.nodesPollInterval),e.nodesPollInterval=null)}function Qi(e){e.logsPollInterval==null&&(e.logsPollInterval=window.setInterval(()=>{e.tab==="logs"&&Fi(e,{quiet:!0})},2e3))}function Zi(e){e.logsPollInterval!=null&&(clearInterval(e.logsPollInterval),e.logsPollInterval=null)}function Yi(e){e.debugPollInterval==null&&(e.debugPollInterval=window.setInterval(()=>{e.tab==="debug"&&gn(e)},3e3))}function Ji(e){e.debugPollInterval!=null&&(clearInterval(e.debugPollInterval),e.debugPollInterval=null)}function Ae(e,t){const n={...t,lastActiveSessionKey:t.lastActiveSessionKey?.trim()||t.sessionKey.trim()||"main"};e.settings=n,vl(n),t.theme!==e.theme&&(e.theme=t.theme,yn(e,Gi(t.theme))),e.applySessionKey=e.settings.lastActiveSessionKey}function Ra(e,t){const n=t.trim();n&&e.settings.lastActiveSessionKey!==n&&Ae(e,{...e.settings,lastActiveSessionKey:n})}function dd(e){if(!window.location.search)return;const t=new URLSearchParams(window.location.search),n=t.get("token"),i=t.get("password"),s=t.get("session"),r=t.get("gatewayUrl");let a=!1;if(n!=null){const c=n.trim();c&&c!==e.settings.token&&Ae(e,{...e.settings,token:c}),t.delete("token"),a=!0}if(i!=null){const c=i.trim();c&&(e.password=c),t.delete("password"),a=!0}if(s!=null){const c=s.trim();c&&(e.sessionKey=c,Ae(e,{...e.settings,sessionKey:c,lastActiveSessionKey:c}))}if(r!=null){const c=r.trim();c&&c!==e.settings.gatewayUrl&&Ae(e,{...e.settings,gatewayUrl:c}),t.delete("gatewayUrl"),a=!0}if(!a)return;const l=new URL(window.location.href);l.search=t.toString(),window.history.replaceState({},"",l.toString())}function ud(e,t){e.tab!==t&&(e.tab=t),t==="onboarding"&&(e.onboarding=!0),t==="chat"&&(e.chatHasAutoScrolled=!1),t==="logs"?Qi(e):Zi(e),t==="debug"?Yi(e):Ji(e),Xi(e),Oa(e,t,!1)}function pd(e,t,n){od({nextTheme:t,applyTheme:()=>{e.theme=t,Ae(e,{...e.settings,theme:t}),yn(e,Gi(t))},context:n,currentTheme:e.theme})}async function Xi(e){e.tab==="overview"&&await Na(e),e.tab==="channels"&&await $d(e),e.tab==="instances"&&await Vi(e),e.tab==="sessions"&&await at(e),e.tab==="cron"&&await es(e),e.tab==="skills"&&await Lt(e),e.tab==="nodes"&&(await mn(e),await Ee(e),await ye(e),await qi(e)),e.tab==="chat"&&(await Sd(e),fn(e,!e.chatHasAutoScrolled)),e.tab==="config"&&(await fa(e),await ye(e)),e.tab==="debug"&&(await gn(e),e.eventLog=e.eventLogBuffer),e.tab==="logs"&&(e.logsAtBottom=!0,await Fi(e,{reset:!0}),da(e,!0))}function fd(){if(typeof window>"u")return"";const e=window.__CLAWDBOT_CONTROL_UI_BASE_PATH__;return typeof e=="string"&&e.trim()?pn(e):bl(window.location.pathname)}function hd(e){e.theme=e.settings.theme??"system",yn(e,Gi(e.theme))}function yn(e,t){if(e.themeResolved=t,typeof document>"u")return;const n=document.documentElement;n.dataset.theme=t,n.style.colorScheme=t}function gd(e){if(typeof window>"u"||typeof window.matchMedia!="function")return;if(e.themeMedia=window.matchMedia("(prefers-color-scheme: dark)"),e.themeMediaHandler=n=>{e.theme==="system"&&yn(e,n.matches?"dark":"light")},typeof e.themeMedia.addEventListener=="function"){e.themeMedia.addEventListener("change",e.themeMediaHandler);return}e.themeMedia.addListener(e.themeMediaHandler)}function vd(e){if(!e.themeMedia||!e.themeMediaHandler)return;if(typeof e.themeMedia.removeEventListener=="function"){e.themeMedia.removeEventListener("change",e.themeMediaHandler);return}e.themeMedia.removeListener(e.themeMediaHandler),e.themeMedia=null,e.themeMediaHandler=null}function md(e,t){if(typeof window>"u")return;const n=aa(window.location.pathname,e.basePath)??"chat";Pa(e,n),Oa(e,n,t)}function bd(e){if(typeof window>"u")return;const t=aa(window.location.pathname,e.basePath);if(!t)return;const i=new URL(window.location.href).searchParams.get("session")?.trim();i&&(e.sessionKey=i,Ae(e,{...e.settings,sessionKey:i,lastActiveSessionKey:i})),Pa(e,t)}function Pa(e,t){e.tab!==t&&(e.tab=t),t==="onboarding"&&(e.onboarding=!0),t==="chat"&&(e.chatHasAutoScrolled=!1),t==="logs"?Qi(e):Zi(e),t==="debug"?Yi(e):Ji(e),e.connected&&Xi(e)}function Oa(e,t,n){if(typeof window>"u")return;const i=Tt(Ni(t,e.basePath)),s=Tt(window.location.pathname),r=new URL(window.location.href);t==="chat"&&e.sessionKey?r.searchParams.set("session",e.sessionKey):r.searchParams.delete("session"),s!==i&&(r.pathname=i),n?window.history.replaceState({},"",r.toString()):window.history.pushState({},"",r.toString())}function yd(e,t,n){if(typeof window>"u")return;const i=new URL(window.location.href);i.searchParams.set("session",t),window.history.replaceState({},"",i.toString())}async function Na(e){await Promise.all([le(e,!1),Vi(e),at(e),It(e),gn(e)])}async function $d(e){await Promise.all([le(e,!0),fa(e),ye(e)])}async function es(e){await Promise.all([le(e,!1),It(e),hn(e)])}function Da(e){return e.chatSending||!!e.chatRunId}function wd(e){const t=e.trim();if(!t)return!1;const n=t.toLowerCase();return n==="/stop"?!0:n==="stop"||n==="esc"||n==="abort"||n==="wait"||n==="exit"}async function Ba(e){e.connected&&(e.chatMessage="",await Rl(e))}function xd(e,t,n){const i=t.trim(),s=!!(n&&n.length>0);!i&&!s||(e.chatQueue=[...e.chatQueue,{id:Di(),text:i,createdAt:Date.now(),attachments:s?n?.map(r=>({...r})):void 0}])}async function Fa(e,t,n){Bi(e);const i=await Ll(e,t,n?.attachments);return!i&&n?.previousDraft!=null&&(e.chatMessage=n.previousDraft),!i&&n?.previousAttachments&&(e.chatAttachments=n.previousAttachments),i&&Ra(e,e.sessionKey),i&&n?.restoreDraft&&n.previousDraft?.trim()&&(e.chatMessage=n.previousDraft),i&&n?.restoreAttachments&&n.previousAttachments?.length&&(e.chatAttachments=n.previousAttachments),fn(e),i&&!e.chatRunId&&za(e),i}async function za(e){if(!e.connected||Da(e))return;const[t,...n]=e.chatQueue;if(!t)return;e.chatQueue=n,await Fa(e,t.text,{attachments:t.attachments})||(e.chatQueue=[t,...e.chatQueue])}function kd(e,t){e.chatQueue=e.chatQueue.filter(n=>n.id!==t)}async function Ad(e,t,n){if(!e.connected)return;const i=e.chatMessage,s=(t??e.chatMessage).trim(),r=e.chatAttachments??[],a=t==null?r:[],l=a.length>0;if(!(!s&&!l)){if(wd(s)){await Ba(e);return}if(t==null&&(e.chatMessage="",e.chatAttachments=[]),Da(e)){xd(e,s,a);return}await Fa(e,s,{previousDraft:t==null?i:void 0,restoreDraft:!!(t&&n?.restoreDraft),attachments:l?a:void 0,previousAttachments:t==null?r:void 0,restoreAttachments:!!(t&&n?.restoreDraft)})}}async function Sd(e){await Promise.all([tt(e),at(e),gi(e)]),fn(e,!0)}const _d=za;function Td(e){const t=ia(e.sessionKey);return t?.agentId?t.agentId:e.hello?.snapshot?.sessionDefaults?.defaultAgentId?.trim()||"main"}function Ed(e,t){const n=pn(e),i=encodeURIComponent(t);return n?`${n}/avatar/${i}?meta=1`:`/avatar/${i}?meta=1`}async function gi(e){if(!e.connected){e.chatAvatarUrl=null;return}const t=Td(e);if(!t){e.chatAvatarUrl=null;return}e.chatAvatarUrl=null;const n=Ed(e.basePath,t);try{const i=await fetch(n,{method:"GET"});if(!i.ok){e.chatAvatarUrl=null;return}const s=await i.json(),r=typeof s.avatarUrl=="string"?s.avatarUrl.trim():"";e.chatAvatarUrl=r||null}catch{e.chatAvatarUrl=null}}const Ua={CHILD:2},Ka=e=>(...t)=>({_$litDirective$:e,values:t});let Ha=class{constructor(t){}get _$AU(){return this._$AM._$AU}_$AT(t,n,i){this._$Ct=t,this._$AM=n,this._$Ci=i}_$AS(t,n){return this.update(t,n)}update(t,n){return this.render(...n)}};const{I:Cd}=rl,or=e=>e,lr=()=>document.createComment(""),ut=(e,t,n)=>{const i=e._$AA.parentNode,s=t===void 0?e._$AB:t._$AA;if(n===void 0){const r=i.insertBefore(lr(),s),a=i.insertBefore(lr(),s);n=new Cd(r,a,e,e.options)}else{const r=n._$AB.nextSibling,a=n._$AM,l=a!==e;if(l){let c;n._$AQ?.(e),n._$AM=e,n._$AP!==void 0&&(c=e._$AU)!==a._$AU&&n._$AP(c)}if(r!==s||l){let c=n._$AA;for(;c!==r;){const p=or(c).nextSibling;or(i).insertBefore(c,s),c=p}}}return n},Le=(e,t,n=e)=>(e._$AI(t,n),e),Md={},Id=(e,t=Md)=>e._$AH=t,Ld=e=>e._$AH,Vn=e=>{e._$AR(),e._$AA.remove()};const cr=(e,t,n)=>{const i=new Map;for(let s=t;s<=n;s++)i.set(e[s],s);return i},Wa=Ka(class extends Ha{constructor(e){if(super(e),e.type!==Ua.CHILD)throw Error("repeat() can only be used in text expressions")}dt(e,t,n){let i;n===void 0?n=t:t!==void 0&&(i=t);const s=[],r=[];let a=0;for(const l of e)s[a]=i?i(l,a):a,r[a]=n(l,a),a++;return{values:r,keys:s}}render(e,t,n){return this.dt(e,t,n).values}update(e,[t,n,i]){const s=Ld(e),{values:r,keys:a}=this.dt(t,n,i);if(!Array.isArray(s))return this.ut=a,r;const l=this.ut??=[],c=[];let p,d,u=0,v=s.length-1,g=0,$=r.length-1;for(;u<=v&&g<=$;)if(s[u]===null)u++;else if(s[v]===null)v--;else if(l[u]===a[g])c[g]=Le(s[u],r[g]),u++,g++;else if(l[v]===a[$])c[$]=Le(s[v],r[$]),v--,$--;else if(l[u]===a[$])c[$]=Le(s[u],r[$]),ut(e,c[$+1],s[u]),u++,$--;else if(l[v]===a[g])c[g]=Le(s[v],r[g]),ut(e,s[u],s[v]),v--,g++;else if(p===void 0&&(p=cr(a,g,$),d=cr(l,u,v)),p.has(l[u]))if(p.has(l[v])){const w=d.get(a[g]),k=w!==void 0?s[w]:null;if(k===null){const E=ut(e,s[u]);Le(E,r[g]),c[g]=E}else c[g]=Le(k,r[g]),ut(e,s[u],k),s[w]=null;g++}else Vn(s[v]),v--;else Vn(s[u]),u++;for(;g<=$;){const w=ut(e,c[$+1]);Le(w,r[g]),c[g++]=w}for(;u<=v;){const w=s[u++];w!==null&&Vn(w)}return this.ut=a,Id(e,c),_e}});function ja(e){const t=e;let n=typeof t.role=="string"?t.role:"unknown";const i=typeof t.toolCallId=="string"||typeof t.tool_call_id=="string",s=t.content,r=Array.isArray(s)?s:null,a=Array.isArray(r)&&r.some(u=>{const g=String(u.type??"").toLowerCase();return g==="toolresult"||g==="tool_result"}),l=typeof t.toolName=="string"||typeof t.tool_name=="string";(i||a||l)&&(n="toolResult");let c=[];typeof t.content=="string"?c=[{type:"text",text:t.content}]:Array.isArray(t.content)?c=t.content.map(u=>({type:u.type||"text",text:u.text,name:u.name,args:u.args||u.arguments})):typeof t.text=="string"&&(c=[{type:"text",text:t.text}]);const p=typeof t.timestamp=="number"?t.timestamp:Date.now(),d=typeof t.id=="string"?t.id:void 0;return{role:n,content:c,timestamp:p,id:d}}function ts(e){const t=e.toLowerCase();return e==="user"||e==="User"?e:e==="assistant"?"assistant":e==="system"?"system":t==="toolresult"||t==="tool_result"||t==="tool"||t==="function"?"tool":e}function qa(e){const t=e,n=typeof t.role=="string"?t.role.toLowerCase():"";return n==="toolresult"||n==="tool_result"}class vi extends Ha{constructor(t){if(super(t),this.it=h,t.type!==Ua.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(t){if(t===h||t==null)return this._t=void 0,this.it=t;if(t===_e)return t;if(typeof t!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(t===this.it)return this._t;this.it=t;const n=[t];return n.raw=n,this._t={_$litType$:this.constructor.resultType,strings:n,values:[]}}}vi.directiveName="unsafeHTML",vi.resultType=1;const mi=Ka(vi);const{entries:Va,setPrototypeOf:dr,isFrozen:Rd,getPrototypeOf:Pd,getOwnPropertyDescriptor:Od}=Object;let{freeze:J,seal:se,create:bi}=Object,{apply:yi,construct:$i}=typeof Reflect<"u"&&Reflect;J||(J=function(t){return t});se||(se=function(t){return t});yi||(yi=function(t,n){for(var i=arguments.length,s=new Array(i>2?i-2:0),r=2;r<i;r++)s[r-2]=arguments[r];return t.apply(n,s)});$i||($i=function(t){for(var n=arguments.length,i=new Array(n>1?n-1:0),s=1;s<n;s++)i[s-1]=arguments[s];return new t(...i)});const qt=X(Array.prototype.forEach),Nd=X(Array.prototype.lastIndexOf),ur=X(Array.prototype.pop),pt=X(Array.prototype.push),Dd=X(Array.prototype.splice),Jt=X(String.prototype.toLowerCase),Gn=X(String.prototype.toString),Qn=X(String.prototype.match),ft=X(String.prototype.replace),Bd=X(String.prototype.indexOf),Fd=X(String.prototype.trim),re=X(Object.prototype.hasOwnProperty),Z=X(RegExp.prototype.test),ht=zd(TypeError);function X(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var n=arguments.length,i=new Array(n>1?n-1:0),s=1;s<n;s++)i[s-1]=arguments[s];return yi(e,t,i)}}function zd(e){return function(){for(var t=arguments.length,n=new Array(t),i=0;i<t;i++)n[i]=arguments[i];return $i(e,n)}}function M(e,t){let n=arguments.length>2&&arguments[2]!==void 0?arguments[2]:Jt;dr&&dr(e,null);let i=t.length;for(;i--;){let s=t[i];if(typeof s=="string"){const r=n(s);r!==s&&(Rd(t)||(t[i]=r),s=r)}e[s]=!0}return e}function Ud(e){for(let t=0;t<e.length;t++)re(e,t)||(e[t]=null);return e}function pe(e){const t=bi(null);for(const[n,i]of Va(e))re(e,n)&&(Array.isArray(i)?t[n]=Ud(i):i&&typeof i=="object"&&i.constructor===Object?t[n]=pe(i):t[n]=i);return t}function gt(e,t){for(;e!==null;){const i=Od(e,t);if(i){if(i.get)return X(i.get);if(typeof i.value=="function")return X(i.value)}e=Pd(e)}function n(){return null}return n}const pr=J(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),Zn=J(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),Yn=J(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),Kd=J(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),Jn=J(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),Hd=J(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),fr=J(["#text"]),hr=J(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns","slot"]),Xn=J(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),gr=J(["accent","accentunder","align","bevelled","close","columnsalign","columnlines","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lspace","lquote","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),Vt=J(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),Wd=se(/\{\{[\w\W]*|[\w\W]*\}\}/gm),jd=se(/<%[\w\W]*|[\w\W]*%>/gm),qd=se(/\$\{[\w\W]*/gm),Vd=se(/^data-[\-\w.\u00B7-\uFFFF]+$/),Gd=se(/^aria-[\-\w]+$/),Ga=se(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),Qd=se(/^(?:\w+script|data):/i),Zd=se(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),Qa=se(/^html$/i),Yd=se(/^[a-z][.\w]*(-[.\w]+)+$/i);var vr=Object.freeze({__proto__:null,ARIA_ATTR:Gd,ATTR_WHITESPACE:Zd,CUSTOM_ELEMENT:Yd,DATA_ATTR:Vd,DOCTYPE_NAME:Qa,ERB_EXPR:jd,IS_ALLOWED_URI:Ga,IS_SCRIPT_OR_DATA:Qd,MUSTACHE_EXPR:Wd,TMPLIT_EXPR:qd});const vt={element:1,text:3,progressingInstruction:7,comment:8,document:9},Jd=function(){return typeof window>"u"?null:window},Xd=function(t,n){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let i=null;const s="data-tt-policy-suffix";n&&n.hasAttribute(s)&&(i=n.getAttribute(s));const r="dompurify"+(i?"#"+i:"");try{return t.createPolicy(r,{createHTML(a){return a},createScriptURL(a){return a}})}catch{return console.warn("TrustedTypes policy "+r+" could not be created."),null}},mr=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function Za(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:Jd();const t=T=>Za(T);if(t.version="3.3.1",t.removed=[],!e||!e.document||e.document.nodeType!==vt.document||!e.Element)return t.isSupported=!1,t;let{document:n}=e;const i=n,s=i.currentScript,{DocumentFragment:r,HTMLTemplateElement:a,Node:l,Element:c,NodeFilter:p,NamedNodeMap:d=e.NamedNodeMap||e.MozNamedAttrMap,HTMLFormElement:u,DOMParser:v,trustedTypes:g}=e,$=c.prototype,w=gt($,"cloneNode"),k=gt($,"remove"),E=gt($,"nextSibling"),C=gt($,"childNodes"),N=gt($,"parentNode");if(typeof a=="function"){const T=n.createElement("template");T.content&&T.content.ownerDocument&&(n=T.content.ownerDocument)}let I,L="";const{implementation:A,createNodeIterator:O,createDocumentFragment:te,getElementsByTagName:He}=n,{importNode:Ot}=i;let Q=mr();t.isSupported=typeof Va=="function"&&typeof N=="function"&&A&&A.createHTMLDocument!==void 0;const{MUSTACHE_EXPR:kn,ERB_EXPR:An,TMPLIT_EXPR:Sn,DATA_ATTR:To,ARIA_ATTR:Eo,IS_SCRIPT_OR_DATA:Co,ATTR_WHITESPACE:fs,CUSTOM_ELEMENT:Mo}=vr;let{IS_ALLOWED_URI:hs}=vr,K=null;const gs=M({},[...pr,...Zn,...Yn,...Jn,...fr]);let W=null;const vs=M({},[...hr,...Xn,...gr,...Vt]);let F=Object.seal(bi(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),ot=null,_n=null;const We=Object.seal(bi(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let ms=!0,Tn=!0,bs=!1,ys=!0,je=!1,Nt=!0,Ce=!1,En=!1,Cn=!1,qe=!1,Dt=!1,Bt=!1,$s=!0,ws=!1;const Io="user-content-";let Mn=!0,lt=!1,Ve={},ce=null;const In=M({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","style","svg","template","thead","title","video","xmp"]);let xs=null;const ks=M({},["audio","video","img","source","image","track"]);let Ln=null;const As=M({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),Ft="http://www.w3.org/1998/Math/MathML",zt="http://www.w3.org/2000/svg",he="http://www.w3.org/1999/xhtml";let Ge=he,Rn=!1,Pn=null;const Lo=M({},[Ft,zt,he],Gn);let Ut=M({},["mi","mo","mn","ms","mtext"]),Kt=M({},["annotation-xml"]);const Ro=M({},["title","style","font","a","script"]);let ct=null;const Po=["application/xhtml+xml","text/html"],Oo="text/html";let U=null,Qe=null;const No=n.createElement("form"),Ss=function(f){return f instanceof RegExp||f instanceof Function},On=function(){let f=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(!(Qe&&Qe===f)){if((!f||typeof f!="object")&&(f={}),f=pe(f),ct=Po.indexOf(f.PARSER_MEDIA_TYPE)===-1?Oo:f.PARSER_MEDIA_TYPE,U=ct==="application/xhtml+xml"?Gn:Jt,K=re(f,"ALLOWED_TAGS")?M({},f.ALLOWED_TAGS,U):gs,W=re(f,"ALLOWED_ATTR")?M({},f.ALLOWED_ATTR,U):vs,Pn=re(f,"ALLOWED_NAMESPACES")?M({},f.ALLOWED_NAMESPACES,Gn):Lo,Ln=re(f,"ADD_URI_SAFE_ATTR")?M(pe(As),f.ADD_URI_SAFE_ATTR,U):As,xs=re(f,"ADD_DATA_URI_TAGS")?M(pe(ks),f.ADD_DATA_URI_TAGS,U):ks,ce=re(f,"FORBID_CONTENTS")?M({},f.FORBID_CONTENTS,U):In,ot=re(f,"FORBID_TAGS")?M({},f.FORBID_TAGS,U):pe({}),_n=re(f,"FORBID_ATTR")?M({},f.FORBID_ATTR,U):pe({}),Ve=re(f,"USE_PROFILES")?f.USE_PROFILES:!1,ms=f.ALLOW_ARIA_ATTR!==!1,Tn=f.ALLOW_DATA_ATTR!==!1,bs=f.ALLOW_UNKNOWN_PROTOCOLS||!1,ys=f.ALLOW_SELF_CLOSE_IN_ATTR!==!1,je=f.SAFE_FOR_TEMPLATES||!1,Nt=f.SAFE_FOR_XML!==!1,Ce=f.WHOLE_DOCUMENT||!1,qe=f.RETURN_DOM||!1,Dt=f.RETURN_DOM_FRAGMENT||!1,Bt=f.RETURN_TRUSTED_TYPE||!1,Cn=f.FORCE_BODY||!1,$s=f.SANITIZE_DOM!==!1,ws=f.SANITIZE_NAMED_PROPS||!1,Mn=f.KEEP_CONTENT!==!1,lt=f.IN_PLACE||!1,hs=f.ALLOWED_URI_REGEXP||Ga,Ge=f.NAMESPACE||he,Ut=f.MATHML_TEXT_INTEGRATION_POINTS||Ut,Kt=f.HTML_INTEGRATION_POINTS||Kt,F=f.CUSTOM_ELEMENT_HANDLING||{},f.CUSTOM_ELEMENT_HANDLING&&Ss(f.CUSTOM_ELEMENT_HANDLING.tagNameCheck)&&(F.tagNameCheck=f.CUSTOM_ELEMENT_HANDLING.tagNameCheck),f.CUSTOM_ELEMENT_HANDLING&&Ss(f.CUSTOM_ELEMENT_HANDLING.attributeNameCheck)&&(F.attributeNameCheck=f.CUSTOM_ELEMENT_HANDLING.attributeNameCheck),f.CUSTOM_ELEMENT_HANDLING&&typeof f.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements=="boolean"&&(F.allowCustomizedBuiltInElements=f.CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements),je&&(Tn=!1),Dt&&(qe=!0),Ve&&(K=M({},fr),W=[],Ve.html===!0&&(M(K,pr),M(W,hr)),Ve.svg===!0&&(M(K,Zn),M(W,Xn),M(W,Vt)),Ve.svgFilters===!0&&(M(K,Yn),M(W,Xn),M(W,Vt)),Ve.mathMl===!0&&(M(K,Jn),M(W,gr),M(W,Vt))),f.ADD_TAGS&&(typeof f.ADD_TAGS=="function"?We.tagCheck=f.ADD_TAGS:(K===gs&&(K=pe(K)),M(K,f.ADD_TAGS,U))),f.ADD_ATTR&&(typeof f.ADD_ATTR=="function"?We.attributeCheck=f.ADD_ATTR:(W===vs&&(W=pe(W)),M(W,f.ADD_ATTR,U))),f.ADD_URI_SAFE_ATTR&&M(Ln,f.ADD_URI_SAFE_ATTR,U),f.FORBID_CONTENTS&&(ce===In&&(ce=pe(ce)),M(ce,f.FORBID_CONTENTS,U)),f.ADD_FORBID_CONTENTS&&(ce===In&&(ce=pe(ce)),M(ce,f.ADD_FORBID_CONTENTS,U)),Mn&&(K["#text"]=!0),Ce&&M(K,["html","head","body"]),K.table&&(M(K,["tbody"]),delete ot.tbody),f.TRUSTED_TYPES_POLICY){if(typeof f.TRUSTED_TYPES_POLICY.createHTML!="function")throw ht('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof f.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw ht('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');I=f.TRUSTED_TYPES_POLICY,L=I.createHTML("")}else I===void 0&&(I=Xd(g,s)),I!==null&&typeof L=="string"&&(L=I.createHTML(""));J&&J(f),Qe=f}},_s=M({},[...Zn,...Yn,...Kd]),Ts=M({},[...Jn,...Hd]),Do=function(f){let x=N(f);(!x||!x.tagName)&&(x={namespaceURI:Ge,tagName:"template"});const _=Jt(f.tagName),B=Jt(x.tagName);return Pn[f.namespaceURI]?f.namespaceURI===zt?x.namespaceURI===he?_==="svg":x.namespaceURI===Ft?_==="svg"&&(B==="annotation-xml"||Ut[B]):!!_s[_]:f.namespaceURI===Ft?x.namespaceURI===he?_==="math":x.namespaceURI===zt?_==="math"&&Kt[B]:!!Ts[_]:f.namespaceURI===he?x.namespaceURI===zt&&!Kt[B]||x.namespaceURI===Ft&&!Ut[B]?!1:!Ts[_]&&(Ro[_]||!_s[_]):!!(ct==="application/xhtml+xml"&&Pn[f.namespaceURI]):!1},de=function(f){pt(t.removed,{element:f});try{N(f).removeChild(f)}catch{k(f)}},Me=function(f,x){try{pt(t.removed,{attribute:x.getAttributeNode(f),from:x})}catch{pt(t.removed,{attribute:null,from:x})}if(x.removeAttribute(f),f==="is")if(qe||Dt)try{de(x)}catch{}else try{x.setAttribute(f,"")}catch{}},Es=function(f){let x=null,_=null;if(Cn)f="<remove></remove>"+f;else{const z=Qn(f,/^[\r\n\t ]+/);_=z&&z[0]}ct==="application/xhtml+xml"&&Ge===he&&(f='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+f+"</body></html>");const B=I?I.createHTML(f):f;if(Ge===he)try{x=new v().parseFromString(B,ct)}catch{}if(!x||!x.documentElement){x=A.createDocument(Ge,"template",null);try{x.documentElement.innerHTML=Rn?L:B}catch{}}const q=x.body||x.documentElement;return f&&_&&q.insertBefore(n.createTextNode(_),q.childNodes[0]||null),Ge===he?He.call(x,Ce?"html":"body")[0]:Ce?x.documentElement:q},Cs=function(f){return O.call(f.ownerDocument||f,f,p.SHOW_ELEMENT|p.SHOW_COMMENT|p.SHOW_TEXT|p.SHOW_PROCESSING_INSTRUCTION|p.SHOW_CDATA_SECTION,null)},Nn=function(f){return f instanceof u&&(typeof f.nodeName!="string"||typeof f.textContent!="string"||typeof f.removeChild!="function"||!(f.attributes instanceof d)||typeof f.removeAttribute!="function"||typeof f.setAttribute!="function"||typeof f.namespaceURI!="string"||typeof f.insertBefore!="function"||typeof f.hasChildNodes!="function")},Ms=function(f){return typeof l=="function"&&f instanceof l};function ge(T,f,x){qt(T,_=>{_.call(t,f,x,Qe)})}const Is=function(f){let x=null;if(ge(Q.beforeSanitizeElements,f,null),Nn(f))return de(f),!0;const _=U(f.nodeName);if(ge(Q.uponSanitizeElement,f,{tagName:_,allowedTags:K}),Nt&&f.hasChildNodes()&&!Ms(f.firstElementChild)&&Z(/<[/\w!]/g,f.innerHTML)&&Z(/<[/\w!]/g,f.textContent)||f.nodeType===vt.progressingInstruction||Nt&&f.nodeType===vt.comment&&Z(/<[/\w]/g,f.data))return de(f),!0;if(!(We.tagCheck instanceof Function&&We.tagCheck(_))&&(!K[_]||ot[_])){if(!ot[_]&&Rs(_)&&(F.tagNameCheck instanceof RegExp&&Z(F.tagNameCheck,_)||F.tagNameCheck instanceof Function&&F.tagNameCheck(_)))return!1;if(Mn&&!ce[_]){const B=N(f)||f.parentNode,q=C(f)||f.childNodes;if(q&&B){const z=q.length;for(let ee=z-1;ee>=0;--ee){const ve=w(q[ee],!0);ve.__removalCount=(f.__removalCount||0)+1,B.insertBefore(ve,E(f))}}}return de(f),!0}return f instanceof c&&!Do(f)||(_==="noscript"||_==="noembed"||_==="noframes")&&Z(/<\/no(script|embed|frames)/i,f.innerHTML)?(de(f),!0):(je&&f.nodeType===vt.text&&(x=f.textContent,qt([kn,An,Sn],B=>{x=ft(x,B," ")}),f.textContent!==x&&(pt(t.removed,{element:f.cloneNode()}),f.textContent=x)),ge(Q.afterSanitizeElements,f,null),!1)},Ls=function(f,x,_){if($s&&(x==="id"||x==="name")&&(_ in n||_ in No))return!1;if(!(Tn&&!_n[x]&&Z(To,x))){if(!(ms&&Z(Eo,x))){if(!(We.attributeCheck instanceof Function&&We.attributeCheck(x,f))){if(!W[x]||_n[x]){if(!(Rs(f)&&(F.tagNameCheck instanceof RegExp&&Z(F.tagNameCheck,f)||F.tagNameCheck instanceof Function&&F.tagNameCheck(f))&&(F.attributeNameCheck instanceof RegExp&&Z(F.attributeNameCheck,x)||F.attributeNameCheck instanceof Function&&F.attributeNameCheck(x,f))||x==="is"&&F.allowCustomizedBuiltInElements&&(F.tagNameCheck instanceof RegExp&&Z(F.tagNameCheck,_)||F.tagNameCheck instanceof Function&&F.tagNameCheck(_))))return!1}else if(!Ln[x]){if(!Z(hs,ft(_,fs,""))){if(!((x==="src"||x==="xlink:href"||x==="href")&&f!=="script"&&Bd(_,"data:")===0&&xs[f])){if(!(bs&&!Z(Co,ft(_,fs,"")))){if(_)return!1}}}}}}}return!0},Rs=function(f){return f!=="annotation-xml"&&Qn(f,Mo)},Ps=function(f){ge(Q.beforeSanitizeAttributes,f,null);const{attributes:x}=f;if(!x||Nn(f))return;const _={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:W,forceKeepAttr:void 0};let B=x.length;for(;B--;){const q=x[B],{name:z,namespaceURI:ee,value:ve}=q,Ze=U(z),Dn=ve;let j=z==="value"?Dn:Fd(Dn);if(_.attrName=Ze,_.attrValue=j,_.keepAttr=!0,_.forceKeepAttr=void 0,ge(Q.uponSanitizeAttribute,f,_),j=_.attrValue,ws&&(Ze==="id"||Ze==="name")&&(Me(z,f),j=Io+j),Nt&&Z(/((--!?|])>)|<\/(style|title|textarea)/i,j)){Me(z,f);continue}if(Ze==="attributename"&&Qn(j,"href")){Me(z,f);continue}if(_.forceKeepAttr)continue;if(!_.keepAttr){Me(z,f);continue}if(!ys&&Z(/\/>/i,j)){Me(z,f);continue}je&&qt([kn,An,Sn],Ns=>{j=ft(j,Ns," ")});const Os=U(f.nodeName);if(!Ls(Os,Ze,j)){Me(z,f);continue}if(I&&typeof g=="object"&&typeof g.getAttributeType=="function"&&!ee)switch(g.getAttributeType(Os,Ze)){case"TrustedHTML":{j=I.createHTML(j);break}case"TrustedScriptURL":{j=I.createScriptURL(j);break}}if(j!==Dn)try{ee?f.setAttributeNS(ee,z,j):f.setAttribute(z,j),Nn(f)?de(f):ur(t.removed)}catch{Me(z,f)}}ge(Q.afterSanitizeAttributes,f,null)},Bo=function T(f){let x=null;const _=Cs(f);for(ge(Q.beforeSanitizeShadowDOM,f,null);x=_.nextNode();)ge(Q.uponSanitizeShadowNode,x,null),Is(x),Ps(x),x.content instanceof r&&T(x.content);ge(Q.afterSanitizeShadowDOM,f,null)};return t.sanitize=function(T){let f=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},x=null,_=null,B=null,q=null;if(Rn=!T,Rn&&(T="<!-->"),typeof T!="string"&&!Ms(T))if(typeof T.toString=="function"){if(T=T.toString(),typeof T!="string")throw ht("dirty is not a string, aborting")}else throw ht("toString is not a function");if(!t.isSupported)return T;if(En||On(f),t.removed=[],typeof T=="string"&&(lt=!1),lt){if(T.nodeName){const ve=U(T.nodeName);if(!K[ve]||ot[ve])throw ht("root node is forbidden and cannot be sanitized in-place")}}else if(T instanceof l)x=Es("<!---->"),_=x.ownerDocument.importNode(T,!0),_.nodeType===vt.element&&_.nodeName==="BODY"||_.nodeName==="HTML"?x=_:x.appendChild(_);else{if(!qe&&!je&&!Ce&&T.indexOf("<")===-1)return I&&Bt?I.createHTML(T):T;if(x=Es(T),!x)return qe?null:Bt?L:""}x&&Cn&&de(x.firstChild);const z=Cs(lt?T:x);for(;B=z.nextNode();)Is(B),Ps(B),B.content instanceof r&&Bo(B.content);if(lt)return T;if(qe){if(Dt)for(q=te.call(x.ownerDocument);x.firstChild;)q.appendChild(x.firstChild);else q=x;return(W.shadowroot||W.shadowrootmode)&&(q=Ot.call(i,q,!0)),q}let ee=Ce?x.outerHTML:x.innerHTML;return Ce&&K["!doctype"]&&x.ownerDocument&&x.ownerDocument.doctype&&x.ownerDocument.doctype.name&&Z(Qa,x.ownerDocument.doctype.name)&&(ee="<!DOCTYPE "+x.ownerDocument.doctype.name+`>
`+ee),je&&qt([kn,An,Sn],ve=>{ee=ft(ee,ve," ")}),I&&Bt?I.createHTML(ee):ee},t.setConfig=function(){let T=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};On(T),En=!0},t.clearConfig=function(){Qe=null,En=!1},t.isValidAttribute=function(T,f,x){Qe||On({});const _=U(T),B=U(f);return Ls(_,B,x)},t.addHook=function(T,f){typeof f=="function"&&pt(Q[T],f)},t.removeHook=function(T,f){if(f!==void 0){const x=Nd(Q[T],f);return x===-1?void 0:Dd(Q[T],x,1)[0]}return ur(Q[T])},t.removeHooks=function(T){Q[T]=[]},t.removeAllHooks=function(){Q=mr()},t}var wi=Za();function ns(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var Ke=ns();function Ya(e){Ke=e}var kt={exec:()=>null};function R(e,t=""){let n=typeof e=="string"?e:e.source,i={replace:(s,r)=>{let a=typeof r=="string"?r:r.source;return a=a.replace(Y.caret,"$1"),n=n.replace(s,a),i},getRegex:()=>new RegExp(n,t)};return i}var eu=(()=>{try{return!!new RegExp("(?<=1)(?<!1)")}catch{return!1}})(),Y={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] +\S/,listReplaceTask:/^\[[ xX]\] +/,listTaskCheckbox:/\[[ xX]\]/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i")},tu=/^(?:[ \t]*(?:\n|$))+/,nu=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,iu=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,Rt=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,su=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,is=/(?:[*+-]|\d{1,9}[.)])/,Ja=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,Xa=R(Ja).replace(/bull/g,is).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),ru=R(Ja).replace(/bull/g,is).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),ss=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,au=/^[^\n]+/,rs=/(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/,ou=R(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",rs).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),lu=R(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,is).getRegex(),$n="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",as=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,cu=R("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",as).replace("tag",$n).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),eo=R(ss).replace("hr",Rt).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",$n).getRegex(),du=R(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",eo).getRegex(),os={blockquote:du,code:nu,def:ou,fences:iu,heading:su,hr:Rt,html:cu,lheading:Xa,list:lu,newline:tu,paragraph:eo,table:kt,text:au},br=R("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",Rt).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",$n).getRegex(),uu={...os,lheading:ru,table:br,paragraph:R(ss).replace("hr",Rt).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",br).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",$n).getRegex()},pu={...os,html:R(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",as).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:kt,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:R(ss).replace("hr",Rt).replace("heading",` *#{1,6} *[^
]`).replace("lheading",Xa).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},fu=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,hu=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,to=/^( {2,}|\\)\n(?!\s*$)/,gu=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,wn=/[\p{P}\p{S}]/u,ls=/[\s\p{P}\p{S}]/u,no=/[^\s\p{P}\p{S}]/u,vu=R(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,ls).getRegex(),io=/(?!~)[\p{P}\p{S}]/u,mu=/(?!~)[\s\p{P}\p{S}]/u,bu=/(?:[^\s\p{P}\p{S}]|~)/u,yu=R(/link|precode-code|html/,"g").replace("link",/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-",eu?"(?<!`)()":"(^^|[^`])").replace("code",/(?<b>`+)[^`]+\k<b>(?!`)/).replace("html",/<(?! )[^<>]*?>/).getRegex(),so=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,$u=R(so,"u").replace(/punct/g,wn).getRegex(),wu=R(so,"u").replace(/punct/g,io).getRegex(),ro="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",xu=R(ro,"gu").replace(/notPunctSpace/g,no).replace(/punctSpace/g,ls).replace(/punct/g,wn).getRegex(),ku=R(ro,"gu").replace(/notPunctSpace/g,bu).replace(/punctSpace/g,mu).replace(/punct/g,io).getRegex(),Au=R("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,no).replace(/punctSpace/g,ls).replace(/punct/g,wn).getRegex(),Su=R(/\\(punct)/,"gu").replace(/punct/g,wn).getRegex(),_u=R(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),Tu=R(as).replace("(?:-->|$)","-->").getRegex(),Eu=R("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",Tu).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),rn=/(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,Cu=R(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",rn).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),ao=R(/^!?\[(label)\]\[(ref)\]/).replace("label",rn).replace("ref",rs).getRegex(),oo=R(/^!?\[(ref)\](?:\[\])?/).replace("ref",rs).getRegex(),Mu=R("reflink|nolink(?!\\()","g").replace("reflink",ao).replace("nolink",oo).getRegex(),yr=/[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,cs={_backpedal:kt,anyPunctuation:Su,autolink:_u,blockSkip:yu,br:to,code:hu,del:kt,emStrongLDelim:$u,emStrongRDelimAst:xu,emStrongRDelimUnd:Au,escape:fu,link:Cu,nolink:oo,punctuation:vu,reflink:ao,reflinkSearch:Mu,tag:Eu,text:gu,url:kt},Iu={...cs,link:R(/^!?\[(label)\]\((.*?)\)/).replace("label",rn).getRegex(),reflink:R(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",rn).getRegex()},xi={...cs,emStrongRDelimAst:ku,emStrongLDelim:wu,url:R(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol",yr).replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,text:R(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol",yr).getRegex()},Lu={...xi,br:R(to).replace("{2,}","*").getRegex(),text:R(xi.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},Gt={normal:os,gfm:uu,pedantic:pu},mt={normal:cs,gfm:xi,breaks:Lu,pedantic:Iu},Ru={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},$r=e=>Ru[e];function be(e,t){if(t){if(Y.escapeTest.test(e))return e.replace(Y.escapeReplace,$r)}else if(Y.escapeTestNoEncode.test(e))return e.replace(Y.escapeReplaceNoEncode,$r);return e}function wr(e){try{e=encodeURI(e).replace(Y.percentDecode,"%")}catch{return null}return e}function xr(e,t){let n=e.replace(Y.findPipe,(r,a,l)=>{let c=!1,p=a;for(;--p>=0&&l[p]==="\\";)c=!c;return c?"|":" |"}),i=n.split(Y.splitPipe),s=0;if(i[0].trim()||i.shift(),i.length>0&&!i.at(-1)?.trim()&&i.pop(),t)if(i.length>t)i.splice(t);else for(;i.length<t;)i.push("");for(;s<i.length;s++)i[s]=i[s].trim().replace(Y.slashPipe,"|");return i}function bt(e,t,n){let i=e.length;if(i===0)return"";let s=0;for(;s<i&&e.charAt(i-s-1)===t;)s++;return e.slice(0,i-s)}function Pu(e,t){if(e.indexOf(t[1])===-1)return-1;let n=0;for(let i=0;i<e.length;i++)if(e[i]==="\\")i++;else if(e[i]===t[0])n++;else if(e[i]===t[1]&&(n--,n<0))return i;return n>0?-2:-1}function kr(e,t,n,i,s){let r=t.href,a=t.title||null,l=e[1].replace(s.other.outputLinkReplace,"$1");i.state.inLink=!0;let c={type:e[0].charAt(0)==="!"?"image":"link",raw:n,href:r,title:a,text:l,tokens:i.inlineTokens(l)};return i.state.inLink=!1,c}function Ou(e,t,n){let i=e.match(n.other.indentCodeCompensation);if(i===null)return t;let s=i[1];return t.split(`
...
… truncated (${n.total} chars, showing first ${n.text.length}).`:"";if(n.text.length>Bu){const l=`<pre class="code-block">${Ku(`${n.text}${i}`)}</pre>`,c=wi.sanitize(l,{ALLOWED_TAGS:Ar,ALLOWED_ATTR:Sr});return t.length<=ei&&Tr(t,c),c}const s=P.parse(`${n.text}${i}`),r=wi.sanitize(s,{ALLOWED_TAGS:Ar,ALLOWED_ATTR:Sr});return t.length<=ei&&Tr(t,r),r}function Ku(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}const Hu=1500,Wu=2e3,lo="Copy as markdown",ju="Copied",qu="Copy failed";async function Vu(e){if(!e)return!1;try{return await navigator.clipboard.writeText(e),!0}catch{return!1}}function Qt(e,t){e.title=t,e.setAttribute("aria-label",t)}function Gu(e){const t=e.label??lo;return o`
  ...
  `}function Qu(e){return Gu({text:()=>e,label:lo})}const Zu={icon:"puzzle",detailKeys:["command","path","url","targetUrl","targetId","ref","element","node","nodeId","id","requestId","to","channelId","guildId","userId","name","query","pattern","messageId"]},Yu={bash:{icon:"wrench",title:"Bash",detailKeys:["command"]},process:{icon:"wrench",title:"Process",detailKeys:["sessionId"]},read:{icon:"fileText",title:"Read",detailKeys:["path"]},write:{icon:"edit",title:"Write",detailKeys:["path"]},edit:{icon:"penLine",title:"Edit",detailKeys:["path"]},attach:{icon:"paperclip",title:"Attach",detailKeys:["path","url","fileName"]},browser:{icon:"globe",title:"Browser",actions:{status:{label:"status"},start:{label:"start"},stop:{label:"stop"},tabs:{label:"tabs"},open:{label:"open",detailKeys:["targetUrl"]},focus:{label:"focus",detailKeys:["targetId"]},close:{label:"close",detailKeys:["targetId"]},snapshot:{label:"snapshot",detailKeys:["targetUrl","targetId","ref","element","format"]},screenshot:{label:"screenshot",detailKeys:["targetUrl","targetId","ref","element"]},navigate:{label:"navigate",detailKeys:["targetUrl","targetId"]},console:{label:"console",detailKeys:["level","targetId"]},pdf:{label:"pdf",detailKeys:["targetId"]},upload:{label:"upload",detailKeys:["paths","ref","inputRef","element","targetId"]},dialog:{label:"dialog",detailKeys:["accept","promptText","targetId"]},act:{label:"act",detailKeys:["request.kind","request.ref","request.selector","request.text","request.value"]}}},canvas:{icon:"image",title:"Canvas",actions:{present:{label:"present",detailKeys:["target","node","nodeId"]},hide:{label:"hide",detailKeys:["node","nodeId"]},navigate:{label:"navigate",detailKeys:["url","node","nodeId"]},eval:{label:"eval",detailKeys:["javaScript","node","nodeId"]},snapshot:{label:"snapshot",detailKeys:["format","node","nodeId"]},a2ui_push:{label:"A2UI push",detailKeys:["jsonlPath","node","nodeId"]},a2ui_reset:{label:"A2UI reset",detailKeys:["node","nodeId"]}}},nodes:{icon:"smartphone",title:"Nodes",actions:{status:{label:"status"},describe:{label:"describe",detailKeys:["node","nodeId"]},pending:{label:"pending"},approve:{label:"approve",detailKeys:["requestId"]},reject:{label:"reject",detailKeys:["requestId"]},notify:{label:"notify",detailKeys:["node","nodeId","title","body"]},camera_snap:{label:"camera snap",detailKeys:["node","nodeId","facing","deviceId"]},camera_list:{label:"camera list",detailKeys:["node","nodeId"]},camera_clip:{label:"camera clip",detailKeys:["node","nodeId","facing","duration","durationMs"]},screen_record:{label:"screen record",detailKeys:["node","nodeId","duration","durationMs","fps","screenIndex"]}}},cron:{icon:"loader",title:"Cron",actions:{status:{label:"status"},list:{label:"list"},add:{label:"add",detailKeys:["job.name","job.id","job.schedule","job.cron"]},update:{label:"update",detailKeys:["id"]},remove:{label:"remove",detailKeys:["id"]},run:{label:"run",detailKeys:["id"]},runs:{label:"runs",detailKeys:["id"]},wake:{label:"wake",detailKeys:["text","mode"]}}},gateway:{icon:"plug",title:"Gateway",actions:{restart:{label:"restart",detailKeys:["reason","delayMs"]},"config.get":{label:"config get"},"config.schema":{label:"config schema"},"config.apply":{label:"config apply",detailKeys:["restartDelayMs"]},"update.run":{label:"update run",detailKeys:["restartDelayMs"]}}},whatsapp_login:{icon:"circle",title:"WhatsApp Login",actions:{start:{label:"start"},wait:{label:"wait"}}},discord:{icon:"messageSquare",title:"Discord",actions:{react:{label:"react",detailKeys:["channelId","messageId","emoji"]},reactions:{label:"reactions",detailKeys:["channelId","messageId"]},sticker:{label:"sticker",detailKeys:["to","stickerIds"]},poll:{label:"poll",detailKeys:["question","to"]},permissions:{label:"permissions",detailKeys:["channelId"]},readMessages:{label:"read messages",detailKeys:["channelId","limit"]},sendMessage:{label:"send",detailKeys:["to","content"]},editMessage:{label:"edit",detailKeys:["channelId","messageId"]},deleteMessage:{label:"delete",detailKeys:["channelId","messageId"]},threadCreate:{label:"thread create",detailKeys:["channelId","name"]},threadList:{label:"thread list",detailKeys:["guildId","channelId"]},threadReply:{label:"thread reply",detailKeys:["channelId","content"]},pinMessage:{label:"pin",detailKeys:["channelId","messageId"]},unpinMessage:{label:"unpin",detailKeys:["channelId","messageId"]},listPins:{label:"list pins",detailKeys:["channelId"]},searchMessages:{label:"search",detailKeys:["guildId","content"]},memberInfo:{label:"member",detailKeys:["guildId","userId"]},roleInfo:{label:"roles",detailKeys:["guildId"]},emojiList:{label:"emoji list",detailKeys:["guildId"]},roleAdd:{label:"role add",detailKeys:["guildId","userId","roleId"]},roleRemove:{label:"role remove",detailKeys:["guildId","userId","roleId"]},channelInfo:{label:"channel",detailKeys:["channelId"]},channelList:{label:"channels",detailKeys:["guildId"]},voiceStatus:{label:"voice",detailKeys:["guildId","userId"]},eventList:{label:"events",detailKeys:["guildId"]},eventCreate:{label:"event create",detailKeys:["guildId","name"]},timeout:{label:"timeout",detailKeys:["guildId","userId"]},kick:{label:"kick",detailKeys:["guildId","userId"]},ban:{label:"ban",detailKeys:["guildId","userId"]}}},slack:{icon:"messageSquare",title:"Slack",actions:{react:{label:"react",detailKeys:["channelId","messageId","emoji"]},reactions:{label:"reactions",detailKeys:["channelId","messageId"]},sendMessage:{label:"send",detailKeys:["to","content"]},editMessage:{label:"edit",detailKeys:["channelId","messageId"]},deleteMessage:{label:"delete",detailKeys:["channelId","messageId"]},readMessages:{label:"read messages",detailKeys:["channelId","limit"]},pinMessage:{label:"pin",detailKeys:["channelId","messageId"]},unpinMessage:{label:"unpin",detailKeys:["channelId","messageId"]},listPins:{label:"list pins",detailKeys:["channelId"]},memberInfo:{label:"member",detailKeys:["userId"]},emojiList:{label:"emoji list"}}}},Ju={fallback:Zu,tools:Yu},co=Ju,Er=co.fallback??{icon:"puzzle"},Xu=co.tools??{};function ep(e){return(e??"tool").trim()}function tp(e){const t=e.replace(/_/g," ").trim();return t?t.split(/\s+/).map(n=>n.length<=2&&n.toUpperCase()===n?n:`${n.at(0)?.toUpperCase()??""}${n.slice(1)}`).join(" "):"Tool"}function np(e){const t=e?.trim();if(t)return t.replace(/_/g," ")}function uo(e){if(e!=null){if(typeof e=="string"){const t=e.trim();if(!t)return;const n=t.split(/\r?\n/)[0]?.trim()??"";return n?n.length>160?`${n.slice(0,157)}…`:n:void 0}if(typeof e=="number"||typeof e=="boolean")return String(e);if(Array.isArray(e)){const t=e.map(i=>uo(i)).filter(i=>!!i);if(t.length===0)return;const n=t.slice(0,3).join(", ");return t.length>3?`${n}…`:n}}}function ip(e,t){if(!e||typeof e!="object")return;let n=e;for(const i of t.split(".")){if(!i||!n||typeof n!="object")return;n=n[i]}return n}function sp(e,t){for(const n of t){const i=ip(e,n),s=uo(i);if(s)return s}}function rp(e){if(!e||typeof e!="object")return;const t=e,n=typeof t.path=="string"?t.path:void 0;if(!n)return;const i=typeof t.offset=="number"?t.offset:void 0,s=typeof t.limit=="number"?t.limit:void 0;return i!==void 0&&s!==void 0?`${n}:${i}-${i+s}`:n}function ap(e){if(!e||typeof e!="object")return;const t=e;return typeof t.path=="string"?t.path:void 0}function op(e,t){if(!(!e||!t))return e.actions?.[t]??void 0}function lp(e){const t=ep(e.name),n=t.toLowerCase(),i=Xu[n],s=i?.icon??Er.icon??"puzzle",r=i?.title??tp(t),a=i?.label??t,l=e.args&&typeof e.args=="object"?e.args.action:void 0,c=typeof l=="string"?l.trim():void 0,p=op(i,c),d=np(p?.label??c);let u;n==="read"&&(u=rp(e.args)),!u&&(n==="write"||n==="edit"||n==="attach")&&(u=ap(e.args));const v=p?.detailKeys??i?.detailKeys??Er.detailKeys??[];return!u&&v.length>0&&(u=sp(e.args,v)),!u&&e.meta&&(u=e.meta),u&&(u=dp(u)),{name:t,icon:s,title:r,label:a,verb:d,detail:u}}function cp(e){const t=[];if(e.verb&&t.push(e.verb),e.detail&&t.push(e.detail),t.length!==0)return t.join(" · ")}function dp(e){return e&&e.replace(/\/Users\/[^/]+/g,"~").replace(/\/home\/[^/]+/g,"~")}const up=80,pp=2,Cr=100;function fp(e){const t=e.trim();if(t.startsWith("{")||t.startsWith("["))try{const n=JSON.parse(t);return"```json\n"+JSON.stringify(n,null,2)+"\n```"}catch{}return e}function hp(e){const t=e.split(`
`),n=t.slice(0,pp),i=n.join(`
`);return i.length>Cr?i.slice(0,Cr)+"…":n.length<t.length?i+"…":i}function gp(e){const t=e,n=vp(t.content),i=[];for(const s of n){const r=String(s.type??"").toLowerCase();(["toolcall","tool_call","tooluse","tool_use"].includes(r)||typeof s.name=="string"&&s.arguments!=null)&&i.push({kind:"call",name:s.name??"tool",args:mp(s.arguments??s.args)})}for(const s of n){const r=String(s.type??"").toLowerCase();if(r!=="toolresult"&&r!=="tool_result")continue;const a=bp(s),l=typeof s.name=="string"?s.name:"tool";i.push({kind:"result",name:l,text:a})}if(qa(e)&&!i.some(s=>s.kind==="result")){const s=typeof t.toolName=="string"&&t.toolName||typeof t.tool_name=="string"&&t.tool_name||"tool",r=ca(e)??void 0;i.push({kind:"result",name:s,text:r})}return i}function Mr(e,t){const n=lp({name:e.name,args:e.args}),i=cp(n),s=!!e.text?.trim(),r=!!t,a=r?()=>{if(s){t(fp(e.text));return}const u=`## ${n.label}

${i?`**Command:** \`${i}\`

`:""}*No output — tool completed successfully.*`;t(u)}:void 0,l=s&&(e.text?.length??0)<=up,c=s&&!l,p=s&&l,d=!s;return o`
    <div
    ...
    >
      ...
      ${c?o`<div class="chat-tool-card__preview mono">${hp(e.text)}</div>`:h}
      ${p?o`<div class="chat-tool-card__inline mono">${e.text}</div>`:h}
    </div>
  `}function vp(e){return Array.isArray(e)?e.filter(Boolean):[]}function mp(e){if(typeof e!="string")return e;const t=e.trim();if(!t||!t.startsWith("{")&&!t.startsWith("["))return e;try{return JSON.parse(t)}catch{return e}}function bp(e){if(typeof e.text=="string")return e.text;if(typeof e.content=="string")return e.content}function yp(e){const n=e.content,i=[];if(Array.isArray(n))for(const s of n){if(typeof s!="object"||s===null)continue;const r=s;if(r.type==="image"){const a=r.source;if(a?.type==="base64"&&typeof a.data=="string"){const l=a.data,c=a.media_type||"image/png",p=l.startsWith("data:")?l:`data:${c};base64,${l}`;i.push({url:p})}else typeof r.url=="string"&&i.push({url:r.url})}else if(r.type==="image_url"){const a=r.image_url;typeof a?.url=="string"&&i.push({url:a.url})}}return i}function $p(e){return o`
  ...
  `}function wp(e,t,n,i){const s=new Date(t).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),r=i?.name??"智能体";return o`
    <div class="chat-group assistant">
      ${us("assistant",i)}
      <div class="chat-group-messages">
  ...
  `}function xp(e,t){const n=ts(e.role),i=t.assistantName??"智能体",s=n==="user"?"你":n==="assistant"?i:n,r=n==="user"?"user":n==="assistant"?"assistant":"other",a=new Date(e.timestamp).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return o`
  ...
  `}function us(e,t){const n=ts(e),i=t?.name?.trim()||"智能体",s=t?.avatar?.trim()||"",r=n==="user"?"我":n==="assistant"?i.charAt(0).toUpperCase()||"助":n==="tool"?"⚙":"?",a=n==="user"?"user":n==="assistant"?"assistant":n==="tool"?"tool":"other";return s&&n==="assistant"?kp(s)?o`<img
        class="chat-avatar ${a}"
        src="${s}"
        alt="${i}"
      />`:o`<div class="chat-avatar ${a}">${s}</div>`:o`<div class="chat-avatar ${a}">${r}</div>`}function kp(e){return/^https?:\/\//i.test(e)||/^data:image\//i.test(e)||/^\//.test(e)}function Ap(e){return e.length===0?h:o`
  ...
  `}function po(e,t,n){const i=e,s=typeof i.role=="string"?i.role:"unknown",r=qa(e)||s.toLowerCase()==="toolresult"||s.toLowerCase()==="tool_result"||typeof i.toolCallId=="string"||typeof i.tool_call_id=="string",a=gp(e),l=a.length>0,c=yp(e),p=c.length>0,d=ca(e),u=t.showReasoning&&s==="assistant"?Tl(e):null,v=d?.trim()?d:null,g=u?Cl(u):null,$=v,w=s==="assistant"&&!!$?.trim(),k=["chat-bubble",w?"has-copy":"",t.isStreaming?"streaming":"","fade-in"].filter(Boolean).join(" ");return!$&&l&&r?o`${a.map(E=>Mr(E,n))}`:!$&&!l&&!p?h:o`
    <div class="${k}">
      ${w?Qu($):h}
      ${Ap(c)}
  ...
  `}var _p=Object.defineProperty,Tp=Object.getOwnPropertyDescriptor,xn=(e,t,n,i)=>{for(var s=i>1?void 0:i?Tp(t,n):t,r=e.length-1,a;r>=0;r--)(a=e[r])&&(s=(i?a(t,n,s):a(s))||s);return i&&s&&_p(t,n,s),s};let st=class extends Xe{constructor(){super(...arguments),this.splitRatio=.6,this.minRatio=.4,this.maxRatio=.7,this.isDragging=!1,this.startX=0,this.startRatio=0,this.handleMouseDown=e=>{this.isDragging=!0,this.startX=e.clientX,this.startRatio=this.splitRatio,this.classList.add("dragging"),document.addEventListener("mousemove",this.handleMouseMove),document.addEventListener("mouseup",this.handleMouseUp),e.preventDefault()},this.handleMouseMove=e=>{if(!this.isDragging)return;const t=this.parentElement;if(!t)return;const n=t.getBoundingClientRect().width,s=(e.clientX-this.startX)/n;let r=this.startRatio+s;r=Math.max(this.minRatio,Math.min(this.maxRatio,r)),this.dispatchEvent(new CustomEvent("resize",{detail:{splitRatio:r},bubbles:!0,composed:!0}))},this.handleMouseUp=()=>{this.isDragging=!1,this.classList.remove("dragging"),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}}render(){return o``}connectedCallback(){super.connectedCallback(),this.addEventListener("mousedown",this.handleMouseDown)}disconnectedCallback(){super.disconnectedCallback(),this.removeEventListener("mousedown",this.handleMouseDown),document.removeEventListener("mousemove",this.handleMouseMove),document.removeEventListener("mouseup",this.handleMouseUp)}};st.styles=zo`
    :host {
      width: 4px;
      cursor: col-resize;
  ...
  `;xn([un({type:Number})],st.prototype,"splitRatio",2);xn([un({type:Number})],st.prototype,"minRatio",2);xn([un({type:Number})],st.prototype,"maxRatio",2);st=xn([ta("resizable-divider")],st);const Ep=5e3;function Cp(e){return e?e.active?o`
      <div class="callout info compaction-indicator compaction-indicator--active">
        ${G.loader} 正在压缩上下文...
      </div>
    `:e.completedAt&&Date.now()-e.completedAt<Ep?o`
        <div class="callout success compaction-indicator compaction-indicator--complete">
          ${G.check} 上下文已压缩
        </div>
      `:h:h}function Mp(){return`att-${Date.now()}-${Math.random().toString(36).slice(2,9)}`}function Ip(e,t){const n=e.clipboardData?.items;if(!n||!t.onAttachmentsChange)return;const i=[];for(let s=0;s<n.length;s++){const r=n[s];r.type.startsWith("image/")&&i.push(r)}if(i.length!==0){e.preventDefault();for(const s of i){const r=s.getAsFile();if(!r)continue;const a=new FileReader;a.onload=()=>{const l=a.result,c={id:Mp(),dataUrl:l,mimeType:r.type},p=t.attachments??[];t.onAttachmentsChange?.([...p,c])},a.readAsDataURL(r)}}}function Lp(e){const t=e.attachments??[];return t.length===0?h:o`
  ...
  `}function Rp(e){const t=e.connected,n=e.sending||e.stream!==null,i=!!(e.canAbort&&e.onAbort),r=e.sessions?.sessions?.find(g=>g.key===e.sessionKey)?.reasoningLevel??"off",a=e.showThinking&&r!=="off",l={name:e.assistantName,avatar:e.assistantAvatar??e.assistantAvatarUrl??null},c=(e.attachments?.length??0)>0,p=e.connected?c?"添加消息或粘贴更多图片...":"发送消息（↩ 发送，Shift+↩ 换行，可粘贴图片）":"连接到网关以开始聊天...",d=e.splitRatio??.6,u=!!(e.sidebarOpen&&e.onCloseSidebar),v=o`
    ...
    >
      ...
      ${Wa(Op(e),g=>g.key,g=>g.kind==="reading-indicator"?$p(l):g.kind==="stream"?wp(g.text,g.startedAt,e.onOpenSidebar,l):g.kind==="group"?xp(g,{onOpenSidebar:e.onOpenSidebar,showReasoning:a,assistantName:e.assistantName,assistantAvatar:l.avatar}):h)}
    </div>
  `;return o`
      ...
      >
        ...
        ${u?o`
              ...
              <div class="chat-sidebar">
                ${Sp({content:e.sidebarContent??null,error:e.sidebarError??null,onClose:e.onCloseSidebar,onViewRawText:()=>{!e.sidebarContent||!e.onOpenSidebar||e.onOpenSidebar(`\`\`\`
${e.sidebarContent}
\`\`\``)}})}
              </div>
            `:h}
  ...
  `}const Ir=200;function Pp(e){const t=[];let n=null;for(const i of e){if(i.kind!=="message"){n&&(t.push(n),n=null),t.push(i);continue}const s=ja(i.message),r=ts(s.role),a=s.timestamp||Date.now();!n||n.role!==r?(n&&t.push(n),n={kind:"group",key:`group:${r}:${i.key}`,role:r,messages:[{message:i.message,key:i.key}],timestamp:a,isStreaming:!1}):n.messages.push({message:i.message,key:i.key})}return n&&t.push(n),t}function Op(e){const t=[],n=Array.isArray(e.messages)?e.messages:[],i=Array.isArray(e.toolMessages)?e.toolMessages:[],s=Math.max(0,n.length-Ir);s>0&&t.push({kind:"message",key:"chat:history:notice",message:{role:"system",content:`显示最近 ${Ir} 条消息（隐藏了 ${s} 条）。`,timestamp:Date.now()}});for(let r=s;r<n.length;r++){const a=n[r],l=ja(a);!e.showThinking&&l.role.toLowerCase()==="toolresult"||t.push({kind:"message",key:Lr(a,r),message:a})}if(e.showThinking)for(let r=0;r<i.length;r++)t.push({kind:"message",key:Lr(i[r],r+n.length),message:i[r]});if(e.stream!==null){const r=`stream:${e.sessionKey}:${e.streamStartedAt??"live"}`;e.stream.trim().length>0?t.push({kind:"stream",key:r,text:e.stream,startedAt:e.streamStartedAt??Date.now()}):t.push({kind:"reading-indicator",key:r})}return Pp(t)}function Lr(e,t){const n=e,i=typeof n.toolCallId=="string"?n.toolCallId:"";if(i)return`tool:${i}`;const s=typeof n.id=="string"?n.id:"";if(s)return`msg:${s}`;const r=typeof n.messageId=="string"?n.messageId:"";if(r)return`msg:${r}`;const a=typeof n.timestamp=="number"?n.timestamp:null,l=typeof n.role=="string"?n.role:"unknown";return a!=null?`msg:${l}:${a}:${t}`:`msg:${l}:${t}`}function fe(e){if(e)return Array.isArray(e.type)?e.type.filter(n=>n!=="null")[0]??e.type[0]:e.type}function fo(e){if(!e)return"";if(e.default!==void 0)return e.default;switch(fe(e)){case"object":return{};case"array":return[];case"boolean":return!1;case"number":case"integer":return 0;case"string":return"";default:return""}}function Pt(e){return e.map(t=>typeof t=="number"?"*":t).join(".")}function ie(e,t){const n=Pt(e),i=t[n];if(i)return i;const s=n.split(".");for(const[r,a]of Object.entries(t)){if(!r.includes("*"))continue;const l=r.split(".");if(l.length!==s.length)continue;let c=!0;for(let p=0;p<s.length;p+=1)if(l[p]!=="*"&&l[p]!==s[p]){c=!1;break}if(c)return a}}function we(e){return e.replace(/_/g," ").replace(/([a-z0-9])([A-Z])/g,"$1 $2").replace(/\s+/g," ").replace(/^./,t=>t.toUpperCase())}function Np(e){const t=Pt(e).toLowerCase();return t.includes("token")||t.includes("password")||t.includes("secret")||t.includes("apikey")||t.endsWith("key")}function Dp(e,t){const n=Pt(e);if(t.has(n))return!0;const i=n.split(".");for(const s of t){if(!s.includes("*"))continue;const r=s.split(".");if(r.length!==i.length)continue;let a=!0;for(let l=0;l<i.length;l+=1)if(r[l]!=="*"&&r[l]!==i[l]){a=!1;break}if(a)return!0}return!1}const Bp=new Set(["title","description","default","nullable"]);function Fp(e){return Object.keys(e??{}).filter(n=>!Bp.has(n)).length===0}function zp(e){if(e===void 0)return"";try{return JSON.stringify(e,null,2)??""}catch{return""}}const rt={chevronDown:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`,plus:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>`,minus:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line></svg>`,trash:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>`,edit:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>`,refresh:o`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 4v6h-6"></path><path d="M1 20v-6h6"></path><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>`};function $e(e){const{schema:t,value:n,path:i,hints:s,unsupported:r,disabled:a,onPatch:l,onDiscoverModels:c}=e,p=e.showLabel??!0,d=fe(t),u=ie(i,s),v=u?.label??t.title??String(i.at(-1)??""),g=v==="undefined"||!v?we(String(i.at(-1)??"")):v,$=u?.help??t.description;if(Pt(i),Dp(i,r))return o`<div class="cfg-field cfg-field--error">
      ...
      `}return Pr({...e,options:w,value:n??t.default})}if(d==="object")return Kp(e);if(d==="array")return Hp(e);if(d==="boolean"){const w=typeof n=="boolean"?n:typeof t.default=="boolean"?t.default:!1;return o`
    ...
    `}return d==="number"||d==="integer"?Up(e):d==="string"?Rr({...e,inputType:"text"}):o`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${g}</div>
      <div class="cfg-field__error">不支持的类型: ${d}。请使用原生模式 (Raw)。</div>
    </div>
  `}function Rr(e){const{schema:t,value:n,path:i,hints:s,disabled:r,onPatch:a,inputType:l}=e,c=e.showLabel??!0,p=ie(i,s),d=p?.label??t.title??we(String(i.at(-1))),u=p?.help??t.description,v=p?.sensitive??Np(i),g=p?.placeholder??(v?"••••":t.default!==void 0?`默认值: ${t.default}`:""),$=n??"";return o`
    <div class="cfg-field">
  ...
  `}function Up(e){const{schema:t,value:n,path:i,hints:s,disabled:r,onPatch:a}=e,l=e.showLabel??!0,c=ie(i,s),p=c?.label??t.title??we(String(i.at(-1))),d=c?.help??t.description,u=n??t.default??"",v=typeof u=="number"?u:0;return o`
    <div class="cfg-field">
  ...
  `}function Hp(e){const{schema:t,value:n,path:i,hints:s,unsupported:r,disabled:a,onPatch:l}=e,c=e.showLabel??!0,p=ie(i,s),d=p?.label??t.title??we(String(i.at(-1))),u=p?.help??t.description,v=Array.isArray(t.items)?t.items[0]:t.items;if(!v)return o`
      <div class="cfg-field cfg-field--error">
  ...
  `}function Or(e){const{schema:t,value:n,path:i,hints:s,unsupported:r,disabled:a,reservedKeys:l,onPatch:c}=e,p=Fp(t),d=Object.entries(n??{}).filter(([u])=>!l.has(u));return o`
    <div class="cfg-map">
      ...
      `:o`
        <div class="cfg-map__items">
          ${d.map(([u,v])=>{const g=[...i,u],$=zp(v);return o`
        ...
        <div class="config-content">
          ${e.formMode==="form"?o`
                ${e.schemaLoading?o`<div class="config-loading">
                    ...
                    </div>`:jp({schema:n.schema,uiHints:e.uiHints,value:e.formValue,disabled:e.loading||!e.formValue,unsupportedPaths:n.unsupportedPaths,onPatch:e.onFormPatch,onDiscoverModels:e.onDiscoverModels,searchQuery:e.searchQuery,activeSection:e.activeSection,activeSubsection:$})}
  ...
  `}function Xp(e){if(!e&&e!==0)return"无";const t=Math.round(e/1e3);if(t<60)return`${t}s`;const n=Math.round(t/60);return n<60?`${n}m`:`${Math.round(n/60)}h`}function ef(e,t){const n=t.snapshot,i=n?.channels;if(!n||!i)return!1;const s=i[e],r=typeof s?.configured=="boolean"&&s.configured,a=typeof s?.running=="boolean"&&s.running,l=typeof s?.connected=="boolean"&&s.connected,p=(n.channelAccounts?.[e]??[]).some(d=>d.configured||d.running||d.connected);return r||a||l||p}function tf(e,t){return t?.[e]?.length??0}function vo(e,t){const n=tf(e,t);return n<2?h:o`<div class="account-count">账号 (${n})</div>`}function nf(e,t){let n=e;for(const i of t){if(!n)return null;const s=fe(n);if(s==="object"){const r=n.properties??{};if(typeof i=="string"&&r[i]){n=r[i];continue}const a=n.additionalProperties;if(typeof i=="string"&&a&&typeof a=="object"){n=a;continue}return null}if(s==="array"){if(typeof i!="number")return null;n=(Array.isArray(n.items)?n.items[0]:n.items)??null;continue}return null}return n}function sf(e,t){const i=(e.channels??{})[t],s=e[t];return(i&&typeof i=="object"?i:null)??(s&&typeof s=="object"?s:null)??{}}function rf(e){const t=go(e.schema),n=t.schema;if(!n)return o`<div class="callout danger">架构不可用。请使用原始数据。</div>`;const i=nf(n,["channels",e.channelId]);if(!i)return o`<div class="callout danger">渠道配置架构不可用。</div>`;const s=e.configValue??{},r=sf(s,e.channelId);return o`
  ...
  `}function xe(e){const{channelId:t,props:n}=e,i=n.configSaving||n.configSchemaLoading;return o`
    <div style="margin-top: 16px;">
      ${n.configSchemaLoading?o`<div class="muted">正在加载配置架构...</div>`:rf({channelId:t,configValue:n.configForm,schema:n.configSchema,uiHints:n.configUiHints,disabled:i,onPatch:n.onConfigPatch})}
  ...
  `}function af(e){const{props:t,discord:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function of(e){const{props:t,googleChat:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function lf(e){const{props:t,imessage:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function cf(e){const{values:t,original:n}=e;return t.name!==n.name||t.displayName!==n.displayName||t.about!==n.about||t.picture!==n.picture||t.banner!==n.banner||t.website!==n.website||t.nip05!==n.nip05||t.lud16!==n.lud16}function df(e){const{state:t,callbacks:n,accountId:i}=e,s=cf(t),r=(l,c,p={})=>{const{type:d="text",placeholder:u,maxLength:v,help:g}=p,$=t.values[l]??"",w=t.fieldErrors[l],k=`nostr-profile-${l}`;return d==="textarea"?o`
  ...
  `}function uf(e){const t={name:e?.name??"",displayName:e?.displayName??"",about:e?.about??"",picture:e?.picture??"",banner:e?.banner??"",website:e?.website??"",nip05:e?.nip05??"",lud16:e?.lud16??""};return{values:t,original:{...t},saving:!1,importing:!1,error:null,success:null,fieldErrors:{},showAdvanced:!!(e?.banner||e?.website||e?.nip05||e?.lud16)}}function Kr(e){return e?e.length<=20?e:`${e.slice(0,8)}...${e.slice(-8)}`:"无"}function pf(e){const{props:t,nostr:n,nostrAccounts:i,accountCountLabel:s,profileFormState:r,profileFormCallbacks:a,onEditProfile:l}=e,c=i[0],p=n?.configured??c?.configured??!1,d=n?.running??c?.running??!1,u=n?.publicKey??c?.publicKey,v=n?.lastStartAt??c?.lastStartAt??null,g=n?.lastError??c?.lastError??null,$=i.length>1,w=r!=null,k=C=>{const N=C.publicKey,I=C.profile,L=I?.displayName??I?.name??C.name??C.accountId;return o`
    ...
    `},E=()=>{if(w&&a)return df({state:r,callbacks:a,accountId:i[0]?.accountId??"default"});const C=c?.profile??n?.profile,{name:N,displayName:I,about:L,picture:A,nip05:O}=C??{},te=N||I||L||A||O;return o`
  ...
  `}function ff(e){const{props:t,signal:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function hf(e){const{props:t,slack:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function gf(e){const{props:t,telegram:n,telegramAccounts:i,accountCountLabel:s}=e,r=i.length>1,a=l=>{const p=l.probe?.bot?.username,d=l.name||l.accountId;return o`
      <div class="account-card">
        <div class="account-card-header">
  ...
  `}function vf(e){const{props:t,whatsapp:n,accountCountLabel:i}=e;return o`
    <div class="card">
  ...
  `}function mf(e){const t=e.snapshot?.channels,n=t?.whatsapp??void 0,i=t?.telegram??void 0,s=t?.discord??null;t?.googlechat;const r=t?.slack??null,a=t?.signal??null,l=t?.imessage??null;t?.feishu,t?.wecom;const c=t?.nostr??null,d=bf(e.snapshot).map((u,v)=>({key:u,enabled:ef(u,e),order:v})).sort((u,v)=>u.enabled!==v.enabled?u.enabled?-1:1:u.order-v.order);return o`
    <section class="grid grid-cols-2">
      ${d.map(u=>yf(u.key,e,{whatsapp:n,telegram:i,discord:s,slack:r,signal:a,imessage:l,nostr:c,channelAccounts:e.snapshot?.channelAccounts??null}))}
    </section>
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"暂无快照。"}
  ...
  `}function bf(e){return e?.channelMeta?.length?e.channelMeta.map(t=>t.id):e?.channelOrder?.length?e.channelOrder:["whatsapp","telegram","discord","googlechat","slack","signal","imessage","feishu","wecom","nostr"]}function yf(e,t,n){const i=vo(e,n.channelAccounts);switch(e){case"whatsapp":return vf({props:t,whatsapp:n.whatsapp,accountCountLabel:i});case"telegram":return gf({props:t,telegram:n.telegram,telegramAccounts:n.channelAccounts?.telegram??[],accountCountLabel:i});case"discord":return af({props:t,discord:n.discord,accountCountLabel:i});case"googlechat":return of({props:t,accountCountLabel:i});case"slack":return hf({props:t,slack:n.slack,accountCountLabel:i});case"signal":return ff({props:t,signal:n.signal,accountCountLabel:i});case"imessage":return lf({props:t,imessage:n.imessage,accountCountLabel:i});case"nostr":{const s=n.channelAccounts?.nostr??[],r=s[0],a=r?.accountId??"default",l=r?.profile??null,c=t.nostrProfileAccountId===a?t.nostrProfileFormState:null,p=c?{onFieldChange:t.onNostrProfileFieldChange,onSave:t.onNostrProfileSave,onImport:t.onNostrProfileImport,onCancel:t.onNostrProfileCancel,onToggleAdvanced:t.onNostrProfileToggleAdvanced}:null;return pf({props:t,nostr:n.nostr,nostrAccounts:s,accountCountLabel:i,profileFormState:c,profileFormCallbacks:p,onEditProfile:()=>t.onNostrProfileEdit(a,l)})}default:return $f(e,t,n.channelAccounts??{})}}function $f(e,t,n){const i=xf(t.snapshot,e),s=t.snapshot?.channels?.[e],r=typeof s?.configured=="boolean"?s.configured:void 0,a=typeof s?.running=="boolean"?s.running:void 0,l=typeof s?.connected=="boolean"?s.connected:void 0,c=typeof s?.lastError=="string"?s.lastError:void 0,p=n[e]??[],d=vo(e,n);return o`
    <div class="card">
  ...
  `}function wf(e){return e?.channelMeta?.length?Object.fromEntries(e.channelMeta.map(t=>[t.id,t])):{}}function xf(e,t){return wf(e)[t]?.label??e?.channelLabels?.[t]??t}const kf=600*1e3;function mo(e){return e.lastInboundAt?Date.now()-e.lastInboundAt<kf:!1}function Af(e){return e.running?"是":mo(e)?"活跃":"否"}function Sf(e){return e.connected===!0?"是":e.connected===!1?"否":mo(e)?"活跃":"无"}function _f(e){const t=Af(e),n=Sf(e);return o`
    <div class="account-card">
      <div class="account-card-header">
  ...
  `}function Tf(e){const t=e.host??"未知",n=e.ip?`(${e.ip})`:"",i=e.mode??"",s=e.version??"";return`${t} ${n} ${i} ${s}`.trim()}function Ef(e){const t=e.ts??null;return t?D(t):"无"}function bo(e){return e?`${Et(e)} (${D(e)})`:"无"}function Cf(e){if(e.totalTokens==null)return"无";const t=e.totalTokens??0,n=e.contextTokens??0;return n?`${t} / ${n}`:String(t)}function Mf(e){if(e==null)return"";try{return JSON.stringify(e,null,2)}catch{return String(e)}}function If(e){const t=e.state??{},n=t.nextRunAtMs?Et(t.nextRunAtMs):"无",i=t.lastRunAtMs?Et(t.lastRunAtMs):"无";return`${t.lastStatus??"无"} · 下次 ${n} · 上次 ${i}`}function Lf(e){const t=e.schedule;return t.kind==="at"?`定期 ${Et(t.atMs)}`:t.kind==="every"?`每隔 ${oa(t.everyMs)}`:`定时 ${t.expr}${t.tz?` (${t.tz})`:""}`}function Rf(e){const t=e.payload;return t.kind==="systemEvent"?`系统: ${t.text}`:`智能体: ${t.message}`}function Pf(e){const t=["last",...e.channels.filter(Boolean)],n=e.form.channel?.trim();n&&!t.includes(n)&&t.push(n);const i=new Set;return t.filter(s=>i.has(s)?!1:(i.add(s),!0))}function Of(e,t){if(t==="last")return"last";const n=e.channelMeta?.find(i=>i.id===t);return n?.label?n.label:e.channelLabels?.[t]??t}function Nf(e){const t=Pf(e);return o`
    <section class="grid grid-cols-2">
      <div class="card">
  ...
  `}function Df(e){const t=e.form;return t.scheduleKind==="at"?o`
      <label class="field" style="margin-top: 12px;">
        <span>运行于</span>
        <input
          type="datetime-local"
  ...
  `}function Bf(e,t){const i=`list-item list-item-clickable${t.runsJobId===e.id?" list-item-selected":""}`;return o`
    <div class=${i} @click=${()=>t.onLoadRuns(e.id)}>
      <div class="list-main">
        <div class="list-title">${e.name}</div>
        <div class="list-sub">${Lf(e)}</div>
        <div class="muted">${Rf(e)}</div>
            ...
            <div class="list" style="margin-top: 12px;">
              ${e.eventLog.map(t=>o`
                  <div class="list-item">
                    ...
                    <div class="list-meta">
                      <pre class="code-block">${Mf(t.payload)}</pre>
                    </div>
                  </div>
                `)}
            </div>
  ...
  `}function Uf(e){return o`
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${e.entries.length===0?o`<div class="muted">暂无上报的实例。</div>`:e.entries.map(t=>Kf(t))}
      </div>
    </section>
  `}function Kf(e){const t=e.lastInputSeconds!=null?`${e.lastInputSeconds} 秒前`:"无",n=e.mode??"未知模式",i=Array.isArray(e.roles)?e.roles.filter(Boolean):[],s=Array.isArray(e.scopes)?e.scopes.filter(Boolean):[],r=s.length>0?s.length>3?`${s.length} 个作用域`:`作用域: ${s.join(", ")}`:null;return o`
    <div class="list-item">
      <div class="list-main">
  ...
  `}const Hr=["trace","debug","info","warn","error","fatal"];function Hf(e){if(!e)return"";const t=new Date(e);return Number.isNaN(t.getTime())?e:t.toLocaleTimeString()}function Wf(e,t){return t?[e.message,e.subsystem,e.raw].filter(Boolean).join(" ").toLowerCase().includes(t):!0}function jf(e){const t=e.filterText.trim().toLowerCase(),n=Hr.some(r=>!e.levelFilters[r]),i=e.entries.filter(r=>r.level&&!e.levelFilters[r.level]?!1:Wf(r,t)),s=t||n;return o`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${e.onScroll}>
        ${i.length===0?o`<div class="muted" style="padding: 12px;">无日志条目。</div>`:i.map(r=>o`
                <div class="log-row">
                  <div class="log-time mono">${Hf(r.time)}</div>
  ...
  `}function qf(e){const t=Jf(e),n=sh(e);return o`
    ${ah(n)}
    ${rh(t)}
    ${Vf(e)}
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${e.nodes.length===0?o`<div class="muted">未找到接入点。</div>`:e.nodes.map(i=>vh(i))}
      </div>
    </section>
  `}function Vf(e){const t=e.devicesList??{pending:[],paired:[]},n=Array.isArray(t.pending)?t.pending:[],i=Array.isArray(t.paired)?t.paired:[];return o`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function Gf(e,t){const n=e.displayName?.trim()||e.deviceId,i=typeof e.ts=="number"?D(e.ts):"无",s=e.role?.trim()?`角色: ${e.role}`:"角色: -",r=e.isRepair?" · 修复":"",a=e.remoteIp?` · ${e.remoteIp}`:"";return o`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function Qf(e,t){const n=e.displayName?.trim()||e.deviceId,i=e.remoteIp?` · ${e.remoteIp}`:"",s=`角色: ${ai(e.roles)}`,r=`范围: ${ai(e.scopes)}`,a=Array.isArray(e.tokens)?e.tokens:[];return o`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${n}</div>
  ...
  `}function Zf(e,t,n){const i=t.revokedAtMs?"已撤销":"活跃",s=`范围: ${ai(t.scopes)}`,r=D(t.rotatedAtMs??t.createdAtMs??t.lastUsedAtMs??null);return o`
  ...
  `}const Se="__defaults__",Wr=[{value:"deny",label:"拒绝"},{value:"allowlist",label:"白名单"},{value:"full",label:"完全开放"}],Yf=[{value:"off",label:"关闭"},{value:"on-miss",label:"未命中匹配时"},{value:"always",label:"总是询问"}];function Jf(e){const t=e.configForm,n=fh(e.nodes),{defaultBinding:i,agents:s}=gh(t),r=!!t,a=e.configSaving||e.configFormMode==="raw";return{ready:r,disabled:a,configDirty:e.configDirty,configLoading:e.configLoading,configSaving:e.configSaving,defaultBinding:i,agents:s,nodes:n,onBindDefault:e.onBindDefault,onBindAgent:e.onBindAgent,onSave:e.onSaveBindings,onLoadConfig:e.onLoadConfig,formMode:e.configFormMode}}function jr(e){return e==="allowlist"||e==="full"||e==="deny"?e:"deny"}function Xf(e){return e==="always"||e==="off"||e==="on-miss"?e:"on-miss"}function eh(e){const t=e?.defaults??{};return{security:jr(t.security),ask:Xf(t.ask),askFallback:jr(t.askFallback??"deny"),autoAllowSkills:!!(t.autoAllowSkills??!1)}}function th(e){const t=e?.agents??{},n=Array.isArray(t.list)?t.list:[],i=[];return n.forEach(s=>{if(!s||typeof s!="object")return;const r=s,a=typeof r.id=="string"?r.id.trim():"";if(!a)return;const l=typeof r.name=="string"?r.name.trim():void 0,c=r.default===!0;i.push({id:a,name:l||void 0,isDefault:c})}),i}function nh(e,t){const n=th(e),i=Object.keys(t?.agents??{}),s=new Map;n.forEach(a=>s.set(a.id,a)),i.forEach(a=>{s.has(a)||s.set(a,{id:a})});const r=Array.from(s.values());return r.length===0&&r.push({id:"main",isDefault:!0}),r.sort((a,l)=>{if(a.isDefault&&!l.isDefault)return-1;if(!a.isDefault&&l.isDefault)return 1;const c=a.name?.trim()?a.name:a.id,p=l.name?.trim()?l.name:l.id;return c.localeCompare(p)}),r}function ih(e,t){return e===Se?Se:e&&t.some(n=>n.id===e)?e:Se}function sh(e){const t=e.execApprovalsForm??e.execApprovalsSnapshot?.file??null,n=!!t,i=eh(t),s=nh(e.configForm,t),r=hh(e.nodes),a=e.execApprovalsTarget;let l=a==="node"&&e.execApprovalsTargetNodeId?e.execApprovalsTargetNodeId:null;a==="node"&&l&&!r.some(u=>u.id===l)&&(l=null);const c=ih(e.execApprovalsSelectedAgent,s),p=c!==Se?(t?.agents??{})[c]??null:null,d=Array.isArray(p?.allowlist)?p.allowlist??[]:[];return{ready:n,disabled:e.execApprovalsSaving||e.execApprovalsLoading,dirty:e.execApprovalsDirty,loading:e.execApprovalsLoading,saving:e.execApprovalsSaving,form:t,defaults:i,selectedScope:c,selectedAgent:p,agents:s,allowlist:d,target:a,targetNodeId:l,targetNodes:r,onSelectScope:e.onExecApprovalsSelectAgent,onSelectTarget:e.onExecApprovalsTargetChange,onPatch:e.onExecApprovalsPatch,onRemove:e.onExecApprovalsRemove,onLoad:e.onLoadExecApprovals,onSave:e.onSaveExecApprovals}}function rh(e){const t=e.nodes.length>0,n=e.defaultBinding??"";return o`
    <section class="card">
      ...
      ${e.ready?o`
            <div class="list" style="margin-top: 16px;">
              ...
              ${e.agents.length===0?o`<div class="muted">未找到智能体。</div>`:e.agents.map(i=>ph(i,e))}
            </div>
  ...
  `}function ah(e){const t=e.ready,n=e.target!=="node"||!!e.targetNodeId;return o`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
  ...
  `}function oh(e){const t=e.targetNodes.length>0,n=e.targetNodeId??"";return o`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
  ...
  `}function dh(e){const t=["agents",e.selectedScope,"allowlist"],n=e.allowlist;return o`
    ...
    <div class="list" style="margin-top: 12px;">
      ${n.length===0?o`<div class="muted">暂无白名单项。</div>`:n.map((i,s)=>uh(e,i,s))}
    </div>
  `}function uh(e,t,n){const i=t.lastUsedAt?D(t.lastUsedAt):"从未使用",s=t.lastUsedCommand?oi(t.lastUsedCommand,120):null,r=t.lastResolvedPath?oi(t.lastResolvedPath,120):null;return o`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function ph(e,t){const n=e.binding??"__default__",i=e.name?.trim()?`${e.name} (${e.id})`:e.id,s=t.nodes.length>0;return o`
    <div class="list-item">
      <div class="list-main">
  ...
  `}const bh=["","off","minimal","low","medium","high"],yh=["","off","on"],$h=[{value:"",label:"继承"},{value:"off",label:"关闭 (显式)"},{value:"on",label:"开启"}],wh=[{value:"",label:"继承"},{value:"off",label:"关闭"},{value:"on",label:"开启"},{value:"stream",label:"流式推理"}],xh={"":"继承",off:"关闭",on:"开启",minimal:"极简",low:"低",medium:"中",high:"高"};function kh(e){if(!e)return"";const t=e.trim().toLowerCase();return t==="z.ai"||t==="z-ai"?"zai":t}function yo(e){return kh(e)==="zai"}function Ah(e){return yo(e)?yh:bh}function Sh(e,t){return!t||!e||e==="off"?e:"on"}function _h(e,t){return e?t&&e==="on"?"low":e:null}function Th(e){const t=e.result?.sessions??[];return o`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      <div class="table" style="margin-top: 16px;">
        ...
        ${t.length===0?o`<div class="muted">未找到会话。</div>`:t.map(n=>Eh(n,e.basePath,e.onPatch,e.onDelete,e.loading))}
      </div>
    </section>
  `}function Eh(e,t,n,i,s){const r=e.updatedAt?D(e.updatedAt):"无",a=e.thinkingLevel??"",l=yo(e.modelProvider),c=Sh(a,l),p=Ah(e.modelProvider),d=e.verboseLevel??"",u=e.reasoningLevel??"",v=e.displayName??e.key,g=e.kind!=="global",$=g?`${Ni("chat",t)}?session=${encodeURIComponent(e.key)}`:null;return o`
    <div class="table-row">
      ...
      <div>
        <select
          .value=${c}
          ?disabled=${s}
          @change=${w=>{const k=w.target.value;n(e.key,{thinkingLevel:_h(k,l)})}}
        >
          ${p.map(w=>o`<option value=${w}>${xh[w]||w}</option>`)}
        </select>
      </div>
      <div>
        <select
      ...
      <div>
        ...
        >
          ${wh.map(w=>o`<option value=${w.value}>${w.label}</option>`)}
        </select>
      </div>
      <div>
  ...
  `}function Ch(e){const t=Math.max(0,e),n=Math.floor(t/1e3);if(n<60)return`${n} 秒`;const i=Math.floor(n/60);return i<60?`${i} 分钟`:`${Math.floor(i/60)} 小时`}function Re(e,t){return t?o`<div class="exec-approval-meta-row"><span>${e}</span><span>${t}</span></div>`:h}function Mh(e){const t=e.execApprovalQueue[0];if(!t)return h;const n=t.request,i=t.expiresAtMs-Date.now(),s=i>0?`剩余时间: ${Ch(i)}`:"已过期",r=e.execApprovalQueue.length;return o`
  ...
  `}function Ti(e){return e?o`<div class="onboarding-message">${e}</div>`:h}function Ih(e,t){return Object.is(e,t)}function Lh(e,t,n){const i=e.options??[];return o`
    <div class="list onboarding-options">
      ${i.map(s=>{const r=Ih(s.value,t);return o`
          <div
  ...
  `}function Rh(e,t,n){const i=Array.isArray(t)?t:[],s=e.options??[];return o`
    <div class="list onboarding-options">
  ...
  `}function Ph(e,t,n){const i=typeof t=="string"?t:"",s=e.sensitive?"password":"text";return o`
    <label class="field">
  ...
  `}function Nh(e,t,n){switch(e.type){case"note":case"action":case"progress":return Ti(e.message);case"select":return Lh(e,t,n);case"multiselect":return Rh(e,t,i=>n(i));case"text":return Ph(e,t,i=>n(i));case"confirm":return Oh(e,t,i=>n(i));case"form":return Dh(e,t,n);default:return Ti(e.message)}}function Dh(e,t,n){const i=t??{},s=e.fields??[],r=(a,l)=>{n(c=>({...c??{},[a]:l}))};return o`
    <div class="onboarding-form">
      ${s.map(a=>{const l=i[a.key];return o`
          <div class="onboarding-field">
            <div class="onboarding-field-label">${a.label}</div>
            ${Bh(a,l,c=>r(a.key,c))}
          </div>
        `})}
    </div>
  `}function Bh(e,t,n){switch(e.type){case"text":case"password":return o`
        <input
          type=${e.type}
          class="input"
          .value=${String(t??"")}
      ...
      `;case"select":return o`
        ...
        >
          ${(e.options??[]).map(i=>o`
              <option value=${i.value} ?selected=${Object.is(i.value,t)}>
                ${i.label}
              </option>
            `)}
        </select>
      `;default:return h}}function Fh(e,t){const n=!e.onboardingWizardBusy,i=t.title??"",s=t.message&&(t.type==="text"||t.type==="select"||t.type==="multiselect");return o`
    <div class="card onboarding-card">
      ...
      <div class="onboarding-step-body">
        ${Nh(t,e.onboardingWizardDraft,e.onWizardDraftChange)}
      </div>
      <div class="onboarding-actions">
        ...
        ${t.supportsBack?o`
              <button class="btn" @click=${()=>e.onWizardBack()} ?disabled=${e.onboardingWizardBusy}>
                上一步
              </button>
            `:h}
  ...
  `}function zh(e){return e.onboardingWizardStatus==="done"?o`
      <div class="card onboarding-card">
    ...
    `:h}function Uh(e){const t=e.onboardingWizardStep;return o`
  ...
  `}function Kh(e){const t=e.report?.skills??[],n=e.filter.trim().toLowerCase(),i=n?t.filter(s=>[s.name,s.description,s.source].join(" ").toLowerCase().includes(n)):t;return o`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function Wh(e,t){const n=Ni(t,e.basePath);return o`
    <a
      href=${n}
      ...
      title=${ri(t)}
    >
      <span class="nav-item__icon" aria-hidden="true">${G[yl(t)]}</span>
      <span class="nav-item__text">${ri(t)}</span>
    </a>
  `}function jh(e){const t=qh(e.sessionKey,e.sessionsResult),n=e.onboarding,i=e.onboarding,s=e.onboarding?!1:e.settings.chatShowThinking,r=e.onboarding?!0:e.settings.chatFocusMode,a=o`<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path><path d="M21 3v5h-5"></path></svg>`,l=o`<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7V4h3"></path><path d="M20 7V4h-3"></path><path d="M4 17v3h3"></path><path d="M20 17v3h-3"></path><circle cx="12" cy="12" r="3"></circle></svg>`;return o`
    <div class="chat-controls">
      <label class="field chat-controls__session">
        <select
          .value=${e.sessionKey}
          ?disabled=${!e.connected}
          @change=${c=>{const p=c.target.value;e.sessionKey=p,e.chatMessage="",e.chatStream=null,e.chatStreamStartedAt=null,e.chatRunId=null,e.resetToolStream(),e.resetChatScroll(),e.applySettings({...e.settings,sessionKey:p,lastActiveSessionKey:p}),e.loadAssistantIdentity(),yd(e,p),tt(e)}}
        >
  ...
  `}function qh(e,t){const n=new Set,i=[],s=t?.sessions?.find(r=>r.key===e);if(n.add(e),i.push({key:e,displayName:s?.displayName}),t?.sessions)for(const r of t.sessions)n.has(r.key)||(n.add(r.key),i.push({key:r.key,displayName:r.displayName}));return i}const Vh=["system","light","dark"];function Gh(e){const t=Math.max(0,Vh.indexOf(e.theme)),n=i=>s=>{const a={element:s.currentTarget};(s.clientX||s.clientY)&&(a.pointerClientX=s.clientX,a.pointerClientY=s.clientY),e.setTheme(i,a)};return o`
  ...
  `}const Jh=/^data:/i,Xh=/^https?:\/\//i;function eg(e){const t=e.agentsList?.agents??[],i=ia(e.sessionKey)?.agentId??e.agentsList?.defaultId??"main",r=t.find(l=>l.id===i)?.identity,a=r?.avatarUrl??r?.avatar;if(a)return Jh.test(a)||Xh.test(a)?a:r?.avatarUrl}function tg(e){if(e.onboarding)return Uh({connected:e.connected,basePath:e.basePath,onboardingWizardStep:e.onboardingWizardStep,onboardingWizardStatus:e.onboardingWizardStatus,onboardingWizardError:e.onboardingWizardError,onboardingWizardBusy:e.onboardingWizardBusy,onboardingWizardDraft:e.onboardingWizardDraft,onWizardStart:()=>{e.handleOnboardingStart()},onWizardNext:()=>{e.handleOnboardingNext()},onWizardBack:()=>{e.handleOnboardingBack()},onWizardCancel:()=>{e.handleOnboardingCancel()},onWizardDraftChange:d=>e.handleOnboardingDraftChange(d),onReconnect:()=>e.connect(),onExit:()=>e.handleOnboardingExit()});const t=e.presenceEntries.length,n=e.sessionsResult?.count??null,i=e.cronStatus?.nextWakeAtMs??null,s=e.connected?null:"已断开与网关的连接。",r=e.tab==="chat",a=r&&(e.settings.chatFocusMode||e.onboarding),l=e.onboarding?!1:e.settings.chatShowThinking,c=eg(e),p=e.chatAvatarUrl??c??null;return o`
    <div class="shell ${r?"shell--chat":""} ${a?"shell--chat-focus":""} ${e.settings.navCollapsed?"shell--nav-collapsed":""} ${e.onboarding?"shell--onboarding":""}">
      ...
      <aside class="nav ${e.settings.navCollapsed?"nav--collapsed":""}">
        ${ml.map(d=>{const u=e.settings.navGroupsCollapsed[d.label]??!1,v=d.tabs.some(g=>g===e.tab);return o`
