# Clawdbot Desktop (v1.0.9)

Clawdbot Desktop 是 Clawdbot 的图形化桌面客户端，支持 Windows、macOS 和 Linux。它内置了 Clawdbot Gateway，提供了一键式的 AI 助手启动体验。
...
- **自动认证**：应用会自动读取 `~/.clawdbot.json` 中的 `gateway.auth.token`，并通过 `--token` 参数传递给 Gateway 进程，确保 UI 和服务端鉴权一致。
- **端口管理**：默认使用端口 `18789`。如果端口被占用，建议先通过任务管理器或终端（`netstat -ano | findstr :18789`）清理残留进程。
- **日志查看**：Gateway 的运行日志保存在用户的 AppData 目录中（例如 `~\AppData\Roaming\clawdbot-desktop\gateway.log`）。
...
- **Gateway 无法启动 (端口占用)**：
  如果提示端口 `18789` 已被占用，请确保没有其他 Clawdbot 实例正在运行。可以使用命令 `pkill -9 -f clawdbot` (Linux/macOS) 或在 Windows 上使用 `Stop-Process` 清理。
