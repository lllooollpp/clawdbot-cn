...
有关完整的 OAuth 流程和存储结构，请参阅 [/concepts/oauth](/concepts/oauth.md)。
...
```
3) 如果 Gateway 在 systemd/launchd 下运行，请将密钥放在 `~/.clawdbot/.env` 中，以便守护进程可以读取它：
```bash
...
有关环境变量继承的详情，请参见[帮助](/help/index.md)（`env.shellEnv`、`~/.clawdbot/.env`、systemd/launchd）。
