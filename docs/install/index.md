...
```
详情：[安装程序内部](/install/installer.md)。
...
```
如果你已全局安装了 libvips（在 macOS 上通常通过 Homebrew 安装），而 `sharp` 安装失败，请强制使用预编译二进制文件：
```bash
...
- Nix: [Nix](/install/nix.md)
- Ansible: [Ansible](/install/ansible.md)
- Bun（仅 CLI）: [Bun](/install/bun.md)
...
修复方法：将其添加到你的 shell 启动文件中（zsh：`~/.zshrc`，bash：`~/.bashrc`）：
```bash
# macOS / Linux
...
- 更新：[更新](/install/updating.md)
- 卸载：[卸载](/install/uninstall.md)
```
