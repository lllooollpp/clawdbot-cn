...
功能详情请参见 [控制 UI](/web/control-ui.md)。
本页面主要介绍绑定模式、安全性和面向网络的接口。

## Webhooks

当 `hooks.enabled=true` 时，网关还会在同一个 HTTP 服务器上暴露一个小型的 webhook 端点。
有关身份验证和数据内容的详情，请参见 [网关配置](/gateway/configuration.md) → `hooks`。
json5
{
  gateway: {
...
将网关保持在环回地址上，并让 Tailscale Serve 代理它：
```json5
{
  gateway: {
    bind: "loopback",
...
- 使用 Serve 时，当 `gateway.auth.allowTailscale` 为 `true`，Tailscale 身份头可以满足身份验证（不需要令牌/密码）。设置 `gateway.auth.allowTailscale: false` 以要求显式凭证。参见 [Tailscale](/gateway/tailscale.md) 和 [安全](/gateway/security.md)。
