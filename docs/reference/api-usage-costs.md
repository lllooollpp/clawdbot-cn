...
有关详细信息和示例，请参阅 [Token 使用与费用](/token-use.md)。
...
有关定价配置，请参阅 [模型](/providers/models.md)，有关显示信息请参阅 [Token 使用与费用](/token-use.md)。
...
有关更多信息，请参阅 [媒体理解](/nodes/media-understanding.md)。
...
有关更多信息，请参阅 [内存](/concepts/memory.md)。
...
参见 [模型 CLI](/cli/models.md)。

### 7) 压缩保护摘要
压缩保护可以使用 **当前模型** 对会话历史进行摘要，当运行时会调用提供者 API。

参见 [会话管理 + 压缩](/reference/session-management-compaction.md)。
