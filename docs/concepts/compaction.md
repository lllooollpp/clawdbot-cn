...
## 配置
有关 `agents.defaults.compaction` 设置的详情，请参阅 [压缩配置与模式](/concepts/compaction.md)。
...
在压缩之前，Clawdbot 可以运行一个 **无声内存刷新** 的步骤，将持久化的笔记存储到磁盘上。详情和配置请参阅 [内存](/concepts/memory.md)。
...
有关修剪的详细信息，请参阅 [/concepts/session-pruning](/concepts/session-pruning.md)。
