...
一个代理循环是代理的完整“真实”运行过程：输入 → 上下文组装 → 模型推理 → 工具执行 → 流式回复 → 持久化。这是将消息转化为动作和最终回复的权威路径，同时保持会话状态的一致性。

在 Clawdbot 中，一个循环是每个会话的单一、序列化的运行过程，并在模型思考、调用工具和流式输出时发出生命周期和流事件。本文档解释了这个真实的循环是如何端到端连接的。
...
- **`tool_result_persist`**：在工具结果被写入会话记录之前进行同步转换。
...
- 最终回复列表中会移除消息工具的重复内容。
- 如果没有可渲染的回复内容，并且某个工具出错，则会发出一个回退的工具错误回复
  （除非消息工具已经发送了用户可见的回复）。
...
- `assistant`：从 pi-agent-core 流式传输的助手增量
- `tool`：从 pi-agent-core 流式传输的工具事件

## 聊天通道处理
- 助手的增量被缓冲为聊天的 `delta` 消息。
- 在 **生命周期结束/错误** 时发出聊天的 `final` 消息。
```
...
- 代理运行时：`agents.defaults.timeoutSeconds` 默认值为 600 秒；在 `runEmbeddedPiAgent` 中通过中止计时器强制执行。
