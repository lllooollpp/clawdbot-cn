...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      "私信安全：默认是配对；未知的私信将获得配对码。",
      `使用此命令批准：${formatCliCommand("clawdbot pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    ...
    return (await prompter.select({
      ...
      options: [
        ...
        { value: "allowlist", label: "白名单 (仅限特定用户)" },
        { value: "open", label: "公开 (公开入站私信)" },
        { value: "disabled", label: "已禁用 (忽略私信)" },
      ],
    })) as DmPolicy;
  };
...
export async function setupChannels(
  ...
  const resolveDisabledHint = (channel: ChannelChoice): string | undefined => {
    ...
    if (!plugin) {
      ...
      if (next.plugins?.enabled === false) return "插件均为禁用";
      return undefined;
    }
  ...
  const ensureBundledPluginEnabled = async (channel: ChannelChoice): Promise<boolean> => {
    ...
    next = result.config;
    if (!result.enabled) {
      await prompter.note(`无法启用 ${channel}: ${result.reason ?? "插件已禁用"}。`, "渠道设置");
      return false;
    }
    const workspaceDir = resolveAgentWorkspaceDir(next, resolveDefaultAgentId(next));
    reloadOnboardingPluginRegistry({
    ...
    if (!getChannelPlugin(channel)) {
      await prompter.note(`${channel} 插件不可用。`, "渠道设置");
      return false;
    }
  ...
  const configureChannel = async (channel: ChannelChoice) => {
    const adapter = getChannelOnboardingAdapter(channel);
    if (!adapter) {
      await prompter.note(`${channel} 尚不支持新手引导。`, "渠道设置");
      return;
    }
  ...
  const handleConfiguredChannel = async (channel: ChannelChoice, label: string) => {
    ...
    if (action === "delete" && !supportsDelete) {
      await prompter.note(`${label} 不支持删除配置条目。`, "移除渠道");
      return;
    }
    ...
    if (action === "delete") {
      const confirmed = await prompter.confirm({
        message: `确认删除 ${label} 帐号 "${accountLabel}"？`,
        initialValue: false,
      });
      if (!confirmed) return;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "现在跳过",
          hint: `您可以稍后通过 \`${formatCliCommand("clawdbot channels add")}\` 添加渠道`,
        },
      ],
